...
public class DASHBOARD_grafiek_AWS extends JPanel {
  ...
  public DASHBOARD_grafiek_AWS() {
    ...
    updated_text =
        ""; // updated";  // unfortunately due to a lack of space no room for writing "updated"
    ...
    System.out.println(
        "--- Screen resolution AWS Dashboard analog: " + width_screen + " x " + height_screen);

    if ((width_screen > 3800) && (height_screen > 2000)) // 4K (3840 x 2160) or above
    {
      instrument_diameter = 360;
      vert_offset = 30;
      fontSize_a = 7;
      fontSize_b = 15;
      fontSize_c = 10;
    ...
    } else if ((width_screen > 1900) && (height_screen > 1000)) // Full HD or above (1920 x 1080)
    {
      ...
      fontSize_d = 12;
      color_digits = color_black; // digits value on the dial
    } else // eg 1600 x 900 or 1366 x 768
    {
    ...
    font_d =
        new Font(
            "Monospaced", Font.ITALIC, fontSize_d); // text's like "PRESSURE MSL", "TEMPERATURE" etc

    // default stroke for main hand (= pointer/arrow air pressure value); will be overwritten (by a
    // dashed line) if the communucation to the sensor is lost
    stroke_main_hand = new BasicStroke(1.0f);

    horz_offset = instrument_diameter - 0; // -20 orig
    marker_circle_diameter_1 =
        instrument_diameter - 26; // eg outside marker circle: 220 - 26 = 194;
    marker_circle_diameter_2 =
        marker_circle_diameter_1 - 10; // eg outside marker circle: 194 - 10 = 184;
    units_start =
        instrument_diameter / 2 - 18; // eg 220 / 2 - 18 = 92   // not the 5 and 10 units markers
    units_end =
        instrument_diameter / 2
  ...
  public void paintComponent(Graphics g) {
    ...
    */
    ...
    if ((getWidth() > 3800) && (getHeight() > 2000)) // 4K or above
    {
      g2d.scale((side / 2) / 400, (side / 2) / 400);
    } else {
      g2d.scale((side / 2) / 200, (side / 2) / 200);
    }
    ...
    if (main.displayed_aws_data_obsolate == true) {
      // color_digits = main.obsolate_color_data_from_aws;                      // // will not be
      // visible antmore on the same color instrument face
      color_digits = color_black;
      ...
      float dash1[] = {
        5.0f
      }; // where the values alternate between the dash length and the gap length. a transparent
      // dash for 5 units, another opaque dash for 5 units
      stroke_main_hand =
    ...
    if (main_RS232_RS422.dashboard_string_last_update_record_date_time.equals("")) {
      ...
      // update_message = updated_text_bottom_screen + " " +
      // main_RS232_RS422.dashboard_string_last_update_record_date_time.replace("-", " ");

      // remove the seconds from the date-time update string "12-Dec-2017 08:12:24 UTC" ->
      // "12-Dec-2017 08:12 UTC"
      String updated_message_short =
          main_RS232_RS422.dashboard_string_last_update_record_date_time.substring(
              0, main_RS232_RS422.dashboard_string_last_update_record_date_time.length() - 7);
      ...
      if (main.AWSR) // e.g. OMC-140 but NOT EUCAWS or APR
      {
        ...
        if (((AWSR_system_hour + 1) % Integer.valueOf(main.AWSR_reporting_interval)) == 0) {
          ...
          if (AWSR_minutes_for_upload <= 30) {
            DASHBOARD_view_AWS.jButton1.setEnabled(true);
          } else {
            DASHBOARD_view_AWS.jButton1.setEnabled(false);
          }
        } else {
  ...
  private void draw_AWS_instrument_face_central_knob(Graphics2D g2d) {
    ...
    g2d.fillOval(
        0 - radius_centre_knob,
        0 - radius_centre_knob,
        diameter_centre_knob,
        diameter_centre_knob); // shift x and y (0, 0) by the radius of the circle in order to
    // correctly center it
  }
  ...
  private void draw_AWS_barometer_hands(Graphics2D g2d, boolean pressure_mode_msl) {
    ...
    if (present_test) {
      if (pressure_mode_msl) {
        reading =
            main_RS232_RS422.dashboard_double_last_update_record_MSL_pressure_ic; // see: function:
        // main_RS232_RS422.RS232_compute_dasboard_values()
      } else {
      ...
      if (reading >= 950 && reading <= 1050) {
        if (reading >= 1000 && reading < 1060) {
          m =
              (reading - 1000.0)
                  / 2; // m = 0 if pointer precisly pointing North; m = 15 if East; m = 30 = South;
          // m = 45 = West
        } else // reading < 1000
        {
          m =
              (1020.0 + ((reading - 950.0) * 2) - reading)
                  / 2; // eg 970 hPa: (1020 + ((970 - 950) * 2) - 970)  / 2= 45
        }
      } else // outside range
      {
      ...
      //
      // NB dashboard_string_last_update_record_date_time set in Function:
      // update_AWS_dasboard_values()
      //
      g2d.setFont(font_c);
      if (main_RS232_RS422.dashboard_string_last_update_record_date_time.equals("")) {
        update_message = updated_text + ": unknown";
      } else {
        // remove the seconds from the update string "12-Dec-2017 08:12:24 UTC" -> "12-Dec-2017
        // 08:12 UTC"
        String updated_message_short =
            main_RS232_RS422.dashboard_string_last_update_record_date_time.substring(
                0, main_RS232_RS422.dashboard_string_last_update_record_date_time.length() - 7);
  ...
  private void draw_AWS_thermometer_hand(Graphics2D g2d) {
    ...
    if (main.air_temp_from_AWS_present) {
      ...
      if (reading >= -60 && reading <= 60) {
        ...
        {
          m =
              (20.0 + ((reading + 50.0) * 2) - reading)
                  / 2; // eg -30 C: (20 + ((-30 + 50) * 2) - -30)  / 2 = 45
        }
      } else // outside range
      {
        m = 30; // South (hand pointing to nowhere)
  ...
  private void draw_AWS_wind_dir_hand(Graphics2D g2d, int wind_mode) {
    ...
    if (present_test) {
      ...
      if (reading >= 0.1 && reading <= 360) // dir = 0 should be impossible
      {
        // if (reading >= 0 && reading < 60)
        // {
        //   m = (reading + 0.0) / 2;                  // m = 0 if pointer precisly pointing North;
        // m = 15 if East; m = 30 = South; m = 45 = West
        // }
        // else // reading < 0
        // {
        //   m = (20.0 + ((reading + 50.0) * 2) - reading) / 2;                    // eg -30 C: (20
        // + ((-30 + 50) * 2) - -30)  / 2 = 45
        // }
    ...
    if (main.SST_from_AWS_present) {
      ...
      if (reading >= -10 && reading <= 40) {
        if (reading >= 15 && reading <= 40) {
          m =
              (reading
                  - 15.0); // m = 0 if pointer precisly pointing North; m = 15 if East; m = 30 =
          // South; m = 45 = West
        } else // reading < -15
        {
  ...
  private void make_arrow(Graphics2D g2d, double m) {
    ...
    xm_r =
        Math.cos(m * Math.PI / 30 - Math.PI / 2)
            * arrow_r; // notch, makes the arrow point more dynamic
    ym_r =
        Math.sin(m * Math.PI / 30 - Math.PI / 2)
            * arrow_r; // notch, makes the arrow point more dynamic
