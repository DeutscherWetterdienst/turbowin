...
public class pop_up_screen extends javax.swing.JFrame {
  ...
  private void OK_button_actionPerformed(
    ...
    this.setAlwaysOnTop(
        false); // reset visual obs pop-up screen option to 'NOT aways on top', otherwise main
    // screen cannot be opened this way
  ...
  private void pop_up_screen_windowClosed(
    ...
    // System.out.println("screen_was_manually_closed_in_pop_up_period = " +
    // main.screen_was_manually_closed_in_pop_up_period);
  ...
  private void initComponents2() {
    ...
    // Opacity
    // NB this.setOpacity(0.8f);                       // NB only possible if JFrame not decorated!
    // (upper 'beam' in screen, so then no screen moving or closing with 'x' possible)

    try {
      // setAlwaysOnTop(true);
      // System.out.println("--- pop-up screen visual obs reminder opened and set to 'Always On
      // Top'");
      ...
      try {
        ...
        main.log_turbowin_system_message(
            "[GENERAL] pop-up screen visual obs reminder: error mouse move for deactivating screensaver ("
                + e.getCause().toString()
                + ")");
      }
    } // try
    catch (SecurityException ex) {
      // main.log_turbowin_system_message("[GENERAL] pop-up screen visual obs reminder: there is no
      // permission to set the value of the always-on-top property (" + ex.getCause().toString() +
      // ")");
      main.log_turbowin_system_message(
          "[GENERAL] pop-up screen visual obs reminder error: (" + ex.getCause().toString() + ")");
    }

    // NB requestFocus(): This is not guaranteed to work, because there are many reasons why an
    // operating system would not allow a frame to have focus
    requestFocus();
