...
public class DASHBOARD_grafiek_APR_radar extends JPanel {
  ...
  public void paintComponent(Graphics g) {
    ...
    // NB calculating instrument_width can't be done in DASHBOARD_grafiek_APR_radar() because from
    ...
    // set new origin to middle screen (especially for backround image option, origin will be
    // updated afterwards)
    ...
    // font_h = new Font("Monospaced", Font.BOLD, fontSize_h);                              // Bf
    // description (right info block)
    font_k =
        new Font(
            "Monospaced",
            Font.BOLD,
            (int) (fontSize_e / 1.5)); // label "satellite image (internet)"
    ...
    if ((DASHBOARD_view_APR_radar.night_vision == false)
      ...
      try {
        ...
        // NB "main.dashboard_background_image": saved in e.g. Function choose_APR dashboard_image()
    ...
    g2d.translate(
        DASHBOARD_view_APR_radar.width_APR_radar_dashboard / 2,
        DASHBOARD_view_APR_radar.height_APR_radar_dashboard
            / 2); // new origin (NB see DASHBOARD_APR_view_radar_componentResizedHandler()
    // [DASHBOARD_view_APR_radar.java])
    ...
    if (DASHBOARD_view_APR_radar.night_vision == true) {
      ...
      // color_digits                     = color_black;
      // color_last_update                = color_black;                    // " measured_at" in
      // case hybrid / Meteo France dashboard
    ...
    */
    ...
    if (main_RS232_RS422.test_record.length() > 8) {
      ...
      measured_at_line =
          main_RS232_RS422.test_record.substring(0, main_RS232_RS422.test_record.length() - 7)
              + " UTC"; // remove the seconds (e.g. "10:12:30 UTC" -> "10:12 UTC")
    }
    ...
    // System.out.println("+++ mybarometer.pressure_msl_corrected = " +
        ...
        || ((main.obsolate_data_flag == false)
            && (main.RS232_connection_mode == 7
                || main.RS232_connection_mode
                    == 8))) // NB 7/8 : Mintaka StarX (temp) linked  // temp data flag
    {
    ...
    if ((main.obsolate_data_flag_II == false)
        || ((main.obsolate_data_flag == false)
            && (main.RS232_connection_mode == 7
                || main.RS232_connection_mode == 8))) // NB 7/8 : Mintaka StarX (temp) linked
    {
    ...
    for (int i = (APR_system_hour_local_db + 1);
        i < (APR_system_hour_local_db + 1 + Integer.valueOf(main.APR_reporting_interval));
        i++) {
      // 24 hours or higher makes no difference for this computation
      if (i % Integer.valueOf(main.APR_reporting_interval) == 0) {
        cal_APR_system_date_time_db.add(
            Calendar.HOUR_OF_DAY, i - APR_system_hour_local_db); // add 1 - 5 hours
        cal_APR_system_date_time_db.getTime();

        // String next_APR_date_time = cal_APR_system_date_time_db.get(Calendar.DAY_OF_MONTH) + " "
        // + main.convert_month(cal_APR_system_date_time_db.get(Calendar.MONTH)) + " " +
        // cal_APR_system_date_time_db.get(Calendar.YEAR) + " " +
        // cal_APR_system_date_time_db.get(Calendar.HOUR_OF_DAY) + ".00 UTC";
        String next_APR_date_time =
            cal_APR_system_date_time_db.get(Calendar.HOUR_OF_DAY) + ":00 UTC";
    ...
    // cal_APR_system_dt = new GregorianCalendar(new SimpleTimeZone(0, "UTC"));   // system date
    // time in UTC of this moment
    // cal_APR_system_dt.getTime();                                               // now effective
    // int APR_system_hour = cal_APR_system_dt.get(Calendar.HOUR_OF_DAY);   // HOUR_OF_DAY is used
    // for the 24-hour clock. E.g., at 10:04:15.250 PM the HOUR_OF_DAY is 22.
    ...
    if (((APR_system_hour_local_db + 1) % Integer.valueOf(main.APR_reporting_interval)) == 0) {
      ...
      // NB NB currentTimeMillis(): returns the difference,in milliseconds, between the current
      // system time and midnight, January 1, 1970 UTC
      // NB cal_APR_system_date_time.computeTime(): Converts calendar field values to the time value
      // (millisecond offset from the Epoch).( Epoch, January 1, 1970 00:00:00.000 GMT (Gregorian))

      // System.out.println("+++ System.currentTimeMillis() = " + System.currentTimeMillis());
      // System.out.println("+++ cal_APR_system_dt.getTimeInMillis() = " +
      // cal_APR_system_dt.getTimeInMillis());
      // System.out.println("+++  APR_system_minutes = " +  APR_system_minutes);

      // long APR_minutes_for_upload = (System.currentTimeMillis() -
      // cal_APR_system_dt.getTimeInMillis()) / (1000 * 60);
      ...
      if (APR_minutes_for_upload <= 30) {
        ...
        // main.jLabel39.setText("--- " + (APR_minutes_for_upload -1) + " minutes to go before next
        // automated upload, you can now add observation data ---");
        ...
        DASHBOARD_view_APR_radar.enable_disable_manual_input_controls_APR_radar(
            true); // waves and SST
      } else {
        // main.jLabel39.setForeground(Color.RED);
        // main.jLabel39.setText("--- more than 30 minutes to go before next automated upload,
        // please do not add observation data --- ");
      ...
      // main.jLabel39.setForeground(Color.RED);
      // main.jLabel39.setText("--- more than 30 minutes to go before next automated upload, please
      // do not add observation data --- ");
      visual_obs_line = "Visual obs: " + "not yet";
      // DASHBOARD_view_APR_radar.jButton1.setEnabled(false);
      DASHBOARD_view_APR_radar.enable_disable_wind_controls_APR_radar(false);
      DASHBOARD_view_APR_radar.enable_disable_manual_input_controls_APR_radar(
    ...
    if (DASHBOARD_view_APR_radar.width_APR_radar_dashboard
        < DASHBOARD_view_APR_radar.height_APR_radar_dashboard) {
      wind_rose_diameter =
          DASHBOARD_view_APR_radar.width_APR_radar_dashboard
    ...
    draw_wind_rose_radar(
        ...
        bf_per_class_radius,
        DASHBOARD_view_APR_radar.beaufort_class_APR);
    ...
    if (myposition.COG_APR_wind >= 0.0
        && myposition.COG_APR_wind <= 360.0) // alwayes based on 10 min. (FM13 en format #101)
    {
    ...
    if (main.ship_deck_color_String.equals("") == false) {
      try {
        DASHBOARD_view_APR_radar.deck_color =
    ...
    if (main.ship_tank_color_String.equals("") == false) {
      try {
        DASHBOARD_view_APR_radar.tank_color =
    ...
    if (course_ok) {
      ...
      switch (main.ship_type_dashboard) {
        case main.YACHT:
          main.myship.draw_yacht(g2d, wind_rose_diameter, DASHBOARD_view_APR_radar.night_vision);
          break;
        case main.FULL_RIGGED_3:
          main.myship.draw_tall_ship(
              g2d, wind_rose_diameter, DASHBOARD_view_APR_radar.night_vision);
          break;
        ...
        case main.GENERAL_CARGO_SHIP:
          main.myship.draw_general_cargo_ship(
              g2d,
              wind_rose_diameter,
              DASHBOARD_view_APR_radar.night_vision,
              DASHBOARD_view_APR_radar.deck_color);
          break;
        ...
        case main.RO_RO_SHIP_2:
          main.myship.draw_ro_ro_ship_II(
              g2d, wind_rose_diameter, DASHBOARD_view_APR_radar.night_vision, relativeLightDir);
          break;
        case main.FERRY:
          main.myship.draw_ferry(g2d, wind_rose_diameter, DASHBOARD_view_APR_radar.night_vision);
          break;
    ...
    {
      wind_dir_present = false;
    } else if (DASHBOARD_view_APR_radar.relative_wind_dir_APR.contains("calm")
        || DASHBOARD_view_APR_radar.relative_wind_dir_APR.contains("variable")) {
      wind_dir_present = false;
    } else {
      try {
    ...
    {
      wind_dir_present = false;
    } else if (DASHBOARD_view_APR_radar.wind_dir_APR.contains("calm")
        || DASHBOARD_view_APR_radar.wind_dir_APR.contains("variable")) {
      wind_dir_present = false;
    } else {
      try {
    ...
    draw_info_blocks =
        ((wind_rose_diameter * 2) + (dist_radar_blocks * 2))
            < DASHBOARD_view_APR_radar
                .width_APR_radar_dashboard; // all of the center drawing panel (JPanel1)
    // draw_info_blocks = ((wind_rose_diameter * 2) + 0) <
    // DASHBOARD_view_APR_radar.width_APR_radar_dashboard; // all of the center drawing panel
    // (JPanel1)

    if (draw_info_blocks) {
      wind_rose_radius = wind_rose_diameter / 2;
      DASHBOARD_view_APR_radar.x1_block =
          -wind_rose_radius - (wind_rose_radius) - dist_radar_blocks;
      ...
      g2d.fill(
          new RoundRectangle2D.Double(
              DASHBOARD_view_APR_radar.x1_block,
              DASHBOARD_view_APR_radar.y1_block,
              block_width,
              block_height,
              20,
              20));
      // g2d.fill(new RoundRectangle2D.Double(x5_block, y5_block, block_width, block_height, 20,
      // 20));
      ...
      g2d.draw(
          new RoundRectangle2D.Double(
              ...
              20,
              20));
      // g2d.draw(new RoundRectangle2D.Double(x5_block, y5_block, block_width, block_height, 20,
      // 20));
    } // if (draw_info_blocks)
      ...
      try {
        ...
        if (main.wind_units.trim().indexOf(main.M_S) != -1) {
          DASHBOARD_view_APR_radar.relative_beaufort_class_APR =
              DASHBOARD_view_APR_radar.convert_ms_to_bf((int) Math.round(P));
        } else {
          DASHBOARD_view_APR_radar.relative_beaufort_class_APR =
              DASHBOARD_view_APR_radar.convert_knots_to_bf((int) Math.round(P));
        }

        // relative wind dir [deg]
        DASHBOARD_view_APR_radar.relative_wind_dir_APR =
            Integer.toString((int) Math.round(Po)); // double rounding then -> int then -> String
        ...
        if (main.wind_units.trim().indexOf(main.M_S) != -1) {
          // DASHBOARD_view_APR_radar.jLabel20.setText("rel wind: " + (int)Math.round(Po) + "\u00B0
          // " + (int)Math.round(P) + " m/s");
          DASHBOARD_view_APR_radar.jLabel20.setText(
              "rel wind: "
                  + DASHBOARD_view_APR_radar.relative_wind_dir_APR
                  + "\u00B0 "
                  + (int) Math.round(P)
