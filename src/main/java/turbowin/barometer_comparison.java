...
public class barometer_comparison extends javax.swing.JFrame {
  ...
  private void initComponents() {
                    ...
                    .addGroup(
                        jPanel2Layout
                            ...
                            .addComponent(
                                ...
                                Short.MAX_VALUE)
                            .addComponent(
                                jLabel7, javax.swing.GroupLayout.DEFAULT_SIZE, 400, Short.MAX_VALUE)
                            .addComponent(
                                jLabel12,
                                javax.swing.GroupLayout.DEFAULT_SIZE,
                                javax.swing.GroupLayout.DEFAULT_SIZE,
    ...
    jPanel3.setPreferredSize(new java.awt.Dimension(647, 124));

    jLabel13.setText("actual height of the ship barometer above the waterline (metres)");

    jLabel14.setText("ship barometer air pressure converted to sea level (hPa)");
    ...
    jLabel11.setText(
        "* optional             ** Maintenance -> Station data                      --- this form is not suitable for the Great Lakes ---");
  ...
  private void Compute_button_actionPerformed(
    ...
    if (main.pressure_reading_msl_yes_no.equals(main.PRESSURE_READING_MSL_YES) == true) {
      ...
      info =
          "this form is for ship barometers indicating air pressure at bridge(sensor) level not for ship barometers indicating air pressure at sea level";
      doorgaan = false;
    }
    ...
    //
    if (doorgaan) {
      if ((local_ship_barometer_reading.equals(""))
          || (local_ship_barometer_reading.length() == 0)) {
        ...
        try {
          ...
          if ((double_local_ship_barometer_reading < 800.0)
              || (double_local_ship_barometer_reading > 1200.0)) {
    ...
    //
    if (doorgaan) {
      if ((local_reference_barometer_reading.equals(""))
          || (local_reference_barometer_reading.length() == 0)) {
        ...
        // convert to double
        try {
          double_local_reference_barometer_reading =
              Double.parseDouble(local_reference_barometer_reading);

          if ((double_local_reference_barometer_reading < 800.0)
              || (double_local_reference_barometer_reading > 1200.0)) {
    ...
    if (doorgaan) {
      if (Compute_Barometer_Comparison_Results() == true) {
        ...
        local_ship_barometer_above_wl =
            Double.toString(round(double_local_ship_barometer_above_wl, 1));
        ...
        local_ship_barometer_sea_level =
            Double.toString(round(double_local_ship_barometer_sea_level, 1));
        jTextField11.setText(local_ship_barometer_sea_level);

        // insert computed instrument error
        local_instrument_error_ship_barometer =
            Double.toString(round(double_local_instrument_error_ship_barometer, 1));
        ...
        local_instrument_correction_ship_barometer =
            Double.toString(round(double_local_instrument_correction_ship_barometer, 1));
        jTextField13.setText(local_instrument_correction_ship_barometer);
      } else {
        info =
            "computation error encountered or ship barometer height above the waterline negative";
        JOptionPane.showMessageDialog(
            null, info, main.APPLICATION_NAME + " error", JOptionPane.WARNING_MESSAGE);
      }
    } // if (doorgaan)
  ...
  private void Export_button_actionPerformed(
    ...
    chooser.setFileSelectionMode(
        JFileChooser
            .FILES_ONLY); // now the user can only select files (directories and files are both
    // still visible)
  ...
  private boolean Compute_Barometer_Comparison_Results() {
    ...
    double_local_ship_barometer_above_wl =
        double_local_keel_sll + double_local_barometer_above_sll - double_local_draft;
    ...
    if (term_2 != 0.0) // exception preventing
    {
      ...
      double_local_instrument_correction_ship_barometer =
          double_local_reference_barometer_reading - double_local_ship_barometer_sea_level;
      double_local_instrument_error_ship_barometer =
          double_local_instrument_correction_ship_barometer * -1;
    } else {
  ...
  private void initBarometerComparisonComponents() {
    ...
    // set TAB order, because the 'outcome text fields'(like the field with computed sea level
    // pressure etc.) must not be included in tabbing
    ArrayList<Component> order = new ArrayList<>(12);
    order.add(jTextField1);
    order.add(jTextField2);
    order.add(jTextField3);
    order.add(jTextField4);
    order.add(jTextField5);
    order.add(jTextField6);
    order.add(jTextField7);
    order.add(jTextField8);
    order.add(jTextField9);
    order.add(jButton1);
    order.add(jButton2);
    order.add(jButton3);
  ...
  public static void main(String args[]) {
    ...
    try {
      ...
      java.util.logging.Logger.getLogger(barometer_comparison.class.getName())
