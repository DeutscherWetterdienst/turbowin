...
public class ship 
{
   ...
   public void draw_tall_ship(Graphics2D g2d, double wind_rose_diameter, boolean night_mode) 
   {
      ...
      num_masts = 3;            // 3, 4, 5
      num_sails = 3;            // lowered sails visible on the yard
      //////////////////////// END TEST /////////////
      ...
      double bow_height_virtual = ship_beam * 2.2;                          // NB factor 'x.x' is indication of the bow curve; quadTo; quadTo; vertical distance from control point to start end end point       
      ...
      x2 = ship_beam / 2.0;
      y2 = y1;
      //double ship_length = Math.abs(y1) * 2.1;                                  // so the length of the main mid section; nb factor 'x.x' depends on the factor used at bow_height_virtual and y
      double ship_length = Math.abs(y1) * 2.3;                                  // so the length of the main mid section; nb factor 'x.x' depends on the factor used at bow_height_virtual and y
      ...
      {
/*         
         // 3 or less masts then the foremost mast (and the others) further to aft compared to a 4 or 5 mast ship
         int start_i = 0;
         int end_i = 0;
         if (num_masts <= 3)
         {
...
*/         
         ...
         //for (int i = start_i; i < end_i; i++)   
         {
            ...
            {
               y_mast = y1 + (dist_between_masts * i);
...
/*               
               for (int k = 0; k < num_sails; k++)
               {
                  ...
                  double y_sail = y1_mast + (k * sail_width);                  // the y coordinate of the upper-left corner of the arc to be drawn.
...
*/
               ...
               g2d.setColor(color_masts_tall_ship);
               g2d.draw(new Line2D.Double(x1_mast, y1_mast, x1_mast, y2_mast)); 
               
               /////// aftmost -mizzen- mast barque
               Shape shape_mast = new Ellipse2D.Double(x_mast, y_mast, diameter_mast, diameter_mast);
               g2d.fill(shape_mast);
            }
            else // not the afmost mast -or- [the aftmost mast (mizzen) but not fore-and-aft rigged]
            {
               y_mast = y1 + (dist_between_masts * i);
               double x1_mast = x1 - (ship_beam / 2);
               ...
               Shape shape_mast = new Ellipse2D.Double(x_mast, y_mast, diameter_mast, diameter_mast);
               g2d.fill(shape_mast);
            } // else (aftmost mast (mizzen) not fore-and-aft rigged)
      ...
      } // if (category.equals(FULL_RIGGED))
...
*/
   ...
   */
   ...
   public final String FULL_RIGGED                                         = "full_rigged";
   public final String BARQUE                                              = "barque";
