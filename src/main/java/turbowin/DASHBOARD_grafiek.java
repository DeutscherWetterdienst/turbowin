...
public class DASHBOARD_grafiek extends JPanel {
  ...
  public void paintComponent(Graphics g) {
    ...
    if ((DASHBOARD_view.night_vision == false)
      ...
      try {
        ...
        // NB "main.dashboard_background_image": saved in Function
        // choose_dashboard_image_barometer() [DASHBOARD_view_AWS_radar.java]
      } catch (Exception e) {
    ...
    } // if (DASHBOARD_view.night_vision == false) etc.
    else // no background image selected
    {
      // background photo (only in GUI_SOT mode and not in night vision mode and not in transparent
      // mode)
      //
      if ((DASHBOARD_view.night_vision == false)
          && (!main.theme_mode.equals(main.THEME_TRANSPARENT))) {
        // Image img1 = new ImageIcon(this.getClass().getResource(main.ICONS_DIRECTORY +
        // main.DASHBOARD_LOGO)).getImage();
        //  NB the line below (Image img1 = ... etc.)  didn't work (image file not found) anymore
        // from 2022 why   ???????
        //
        //        work arround: only when the following lines were added it was ok again
        //        System.out.println("+++ " + "this.getClass() = " + this.getClass());
        //        System.out.println("+++ " + "this.getClass().getResource(main.ICONS_DIRECTORY +
        // main.DASHBOARD_LOGO)) = " + this.getClass().getResource(main.ICONS_DIRECTORY +
        // main.DASHBOARD_LOGO));
        //
        // Image img1 = new ImageIcon(this.getClass().getResource(main.ICONS_DIRECTORY +
        // main.DASHBOARD_LOGO)).getImage();
        //
        // to be sure code was changed to the following -alternative- line:
        Image img1 =
            Toolkit.getDefaultToolkit()
                .getImage(this.getClass().getResource(main.ICONS_DIRECTORY + main.DASHBOARD_LOGO));

        // scale the image to cover a the complete area of the drawing surface
        // g2d.drawImage(img1, 0, 0, getWidth(), getHeight(), 0, 0, img1.getWidth(null),
    ...
    if (main.displayed_barometer_data_obsolate == true) {
      color_digits =
          main.obsolate_color_data_from_aws; // will not be visible antmore on the same color
      // instrument face
  ...
  private void draw_barometer_hands(Graphics2D g2d) {
    ...
    //
    reading = main_RS232_RS422.dashboard_double_last_update_record_pressure_ic; // see: function:
    ...
    if (reading >= 950 && reading <= 1050) {
      if (reading >= 1000 && reading < 1060) {
        m =
            (reading - 1000.0)
                / 2; // m = 0 if pointer precisly pointing North; m = 15 if East; m = 30 = South; m
        // = 45 = West
      } else // reading < 1000
      {
        m =
    ...
    xm_r =
        Math.cos(m * Math.PI / 30 - Math.PI / 2) * 73
            + 0; // notch, makes the arrow point more dynamic
    ym_r =
        Math.sin(m * Math.PI / 30 - Math.PI / 2) * 73
            + 0; // notch, makes the arrow point more dynamic
    ...
    if (main_RS232_RS422.dashboard_string_last_update_record_date_time.equals("")) {
      ...
      // remove the seconds from the update string "12-Dec-2017 08:12:24 UTC" -> "12-Dec-2017 08:12
      // UTC"
      String updated_message_short =
          main_RS232_RS422.dashboard_string_last_update_record_date_time.substring(
              0, main_RS232_RS422.dashboard_string_last_update_record_date_time.length() - 7);
    ...
    if (reading > 900.0 && reading < 1100.0) // reading must be a valid value
    {
      reading_ppp =
          reading
              - main_RS232_RS422
                  .dashboard_double_last_update_record_ppp; // eg reading = 1020.0 and ppp = 0.7 ->
      // reading_ppp (reading 3 hours before)
      // = 1019.3
      ...
      if (reading_ppp >= 950 && reading_ppp <= 1050) {
        if (reading_ppp >= 1000 && reading_ppp < 1060) {
          h =
              (reading_ppp - 1000.0)
                  / 2; // h = 0 if pointer precisly pointing North; m = 15 if East; m = 30 = South;
          // m = 45 = West
        } else // reading < 1000
        {
          h =
              (1020.0 + ((reading_ppp - 950.0) * 2) - reading_ppp)
                  / 2; // eg 970 hPa: (1020 + ((970 - 950) * 2) - 970)  / 2= 45
        }
