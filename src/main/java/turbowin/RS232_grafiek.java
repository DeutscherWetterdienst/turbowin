...
public class RS232_grafiek extends JPanel {
  ...
  public RS232_grafiek() {
    ...
    // JOptionPane.showMessageDialog(null, "in RS232_AWS_grafiek", "test",
  ...
  public void paintComponent(Graphics g) {
    ...
    double afstand_aanduiding_naar_marker =
        3; // distance between end of indication and begin of marker
    ...
    int aantal_secondary_lines; // number of secondary (help/intermidiate) lines between two markers
    // (e.g. 10 help lines between 1000 hPa and 1010 hPa)

    Point2D links_boven_grafiek = new Point2D.Double(linker_kantlijn_margin, boven_kantlijn_margin);
    Point2D links_onder_grafiek =
        new Point2D.Double(linker_kantlijn_margin, getHeight() - onder_kantlijn_margin);

    Point2D rechts_boven_grafiek =
        new Point2D.Double(getWidth() - rechter_kantlijn_margin, boven_kantlijn_margin);
    Point2D rechts_onder_grafiek =
        new Point2D.Double(
            getWidth() - rechter_kantlijn_margin, getHeight() - onder_kantlijn_margin);

    // graph top left (a)
    Point2D a_links_boven_grafiek = links_boven_grafiek;
    Point2D a_links_onder_grafiek =
        new Point2D.Double(linker_kantlijn_margin, getHeight() / 2 - onder_kantlijn_margin);
    Point2D a_rechts_boven_grafiek =
        new Point2D.Double(getWidth() / 2 - rechter_kantlijn_margin, boven_kantlijn_margin);
    Point2D a_rechts_onder_grafiek =
        new Point2D.Double(
            getWidth() / 2 - rechter_kantlijn_margin, getHeight() / 2 - onder_kantlijn_margin);

    // graph top right (b)
    Point2D b_links_boven_grafiek =
        new Point2D.Double(getWidth() / 2 + linker_kantlijn_margin, boven_kantlijn_margin);
    Point2D b_links_onder_grafiek =
        new Point2D.Double(
            getWidth() / 2 + linker_kantlijn_margin, getHeight() / 2 - onder_kantlijn_margin);
    Point2D b_rechts_boven_grafiek =
        new Point2D.Double(getWidth() - rechter_kantlijn_margin, boven_kantlijn_margin);
    Point2D b_rechts_onder_grafiek =
        new Point2D.Double(
            getWidth() - rechter_kantlijn_margin, getHeight() / 2 - onder_kantlijn_margin);

    // graph bottom left (c)
    Point2D c_links_boven_grafiek =
        new Point2D.Double(linker_kantlijn_margin, getHeight() / 2 + boven_kantlijn_margin);
    Point2D c_links_onder_grafiek =
        new Point2D.Double(linker_kantlijn_margin, getHeight() - onder_kantlijn_margin);
    Point2D c_rechts_boven_grafiek =
        new Point2D.Double(
            getWidth() / 2 - rechter_kantlijn_margin, getHeight() / 2 + boven_kantlijn_margin);
    Point2D c_rechts_onder_grafiek =
        new Point2D.Double(
            getWidth() / 2 - rechter_kantlijn_margin, getHeight() - onder_kantlijn_margin);

    // graph bottom right (d)
    Point2D d_links_boven_grafiek =
        new Point2D.Double(
            getWidth() / 2 + linker_kantlijn_margin, getHeight() / 2 + boven_kantlijn_margin);
    Point2D d_links_onder_grafiek =
        new Point2D.Double(
            getWidth() / 2 + linker_kantlijn_margin, getHeight() - onder_kantlijn_margin);
    Point2D d_rechts_boven_grafiek =
        new Point2D.Double(
            getWidth() - rechter_kantlijn_margin, getHeight() / 2 + boven_kantlijn_margin);
    // Point2D d_rechts_onder_grafiek = new Point2D.Double(getWidth() - rechter_kantlijn_margin,
    // getHeight() - onder_kantlijn_margin);
    ...
    // {
    //   Color color_very_light_gray  = new Color(204, 204, 204);      // RGB 204,204,204 same as
    // used in high contrat theme (secondary2 in class ContrastTheme)
    ...
    if (RS232_view.night_vision == true) {
      ...
      color_raster_orange =
          new Color(
              255, 0, 0); // RGB 204,204,204 same as used in high contrat theme (secondary2 in class
      // ContrastTheme)
    ...
    if ((main.mode_grafiek).equals(main.MODE_ALL_PARAMETERS)) // 4 graphs view
    {
      ...
      for (int k = 0; k < 4; k++) {
        ...
        if ((RS232_view.mode_tijd_periode).equals(RS232_view.MODE_DAY)) {
          ...
          aantal_time_markers_x_as =
              29; // 7 days every 6 hours (so 4 per dag), actually  30 markers if source also
          // included
        }
        ...
        if ((RS232_view.mode_tijd_periode).equals(RS232_view.MODE_DAY)) {
          ...
          aantal_5_minuten_perioden_tussen_2_markers =
              72; // 12 * 6 (12 blokjes -van 5 minuten per uur- en dan per 6 uur)
        }
        ...
        for (int i = 0; i <= aantal_time_markers_x_as; i++) {
          ...
          if (lo_grafiek != null) {
            marker_begin_x_as =
                new Point2D.Double(x_pos_marker, lo_grafiek.getY() + lengte_x_as_mark);
          ...
          if ((RS232_view.mode_tijd_periode).equals(RS232_view.MODE_DAY)) {
            ...
            cal_grafiek_datum_tijd.add(
                Calendar.HOUR_OF_DAY, -(aantal_time_markers_x_as * 6) + 6 + (i * 6));
          }
          ...
          if (lo_grafiek != null) {
            ...
            g2.drawString(
                uur_aanduiding,
                (int) (x_pos_marker - stringWidth_x / 2),
                (int) (lo_grafiek.getY() + lengte_x_as_mark + ascent_x));
          }
          ...
          if (((RS232_view.mode_tijd_periode).equals(RS232_view.MODE_DAY)
                  && (int_uur_aanduiding == 0 || int_uur_aanduiding == 12))
              || ((RS232_view.mode_tijd_periode).equals(RS232_view.MODE_WEEK))
                  && (int_uur_aanduiding == 0
                      || int_uur_aanduiding == 1
                      || int_uur_aanduiding == 2
                      || int_uur_aanduiding == 3
                      || int_uur_aanduiding == 4
                      || int_uur_aanduiding == 5)) {
            ...
            Rectangle2D bounds_datum =
                font_datum.getStringBounds(test_aanduiding_datum, context_datum);
            double ascent_datum =
                -bounds_datum.getY(); // ascent = helling/steilte (hoogte characters)

            String dag_van_de_week_aanduiding =
                RS232_view.sdf7.format(cal_grafiek_datum_tijd.getTime());
            // String datum_aanduiding = RS232_view.sdf6.format(cal_grafiek_datum_tijd.getTime()) +
            // " UTC";
            String datum_aanduiding = RS232_view.sdf6.format(cal_grafiek_datum_tijd.getTime());

            if (i != aantal_time_markers_x_as) // niet bij de allerlaaste x-as marker (komt dan
            // voorbij de rechter y-as)
            {
              if (lo_grafiek != null) {
                // eg Thuesday
                // g2.drawString(dag_van_de_week_aanduiding, (int)(x_pos_marker),
                // (int)(lo_grafiek.getY() + lengte_x_as_mark + (2 * ascent_datum)));
                g2.drawString(
                    dag_van_de_week_aanduiding,
                    (int) (x_pos_marker),
                    (int) (lo_grafiek.getY() + lengte_x_as_mark + (2.5 * ascent_datum)));

                // eg 12-05-2016
                // g2.drawString(datum_aanduiding, (int)(x_pos_marker), (int)(lo_grafiek.getY() +
                // lengte_x_as_mark + (3 * ascent_datum)));
                g2.drawString(
                    datum_aanduiding,
                    (int) (x_pos_marker),
                    (int) (lo_grafiek.getY() + lengte_x_as_mark + (3.5 * ascent_datum)));
              }
        ...
        for (int i = 0; i <= aantal_parameter_markers_y_as; i++) {
          if ((k != 1) || (k == 1 && i % 3 == 0)) // if wind dir plot only every 30 degrees
          {
            ...
            if (lo_grafiek != null) {
              marker_begin_linker_y_as =
                  new Point2D.Double(lo_grafiek.getX() - lengte_y_as_mark, y_pos_marker);
              marker_eind_linker_y_as = new Point2D.Double(lo_grafiek.getX(), y_pos_marker);
            }

            line_marker_linker_y_as =
                new Line2D.Double(marker_begin_linker_y_as, marker_eind_linker_y_as);
            ...
            if (marker_begin_linker_y_as != null) {
              g2.drawString(
                  aanduiding,
                  (int)
                      (marker_begin_linker_y_as.getX()
                          - stringWidth_y
                          - afstand_aanduiding_naar_marker),
                  (int) (y_pos_marker + ascent_y / 2));
            }
            ...
            if (ro_grafiek != null) {
              marker_eind_rechter_y_as =
                  new Point2D.Double(ro_grafiek.getX() + lengte_y_as_mark, y_pos_marker);
            }

            if (marker_begin_rechter_y_as != null) {
              line_marker_rechter_y_as =
                  new Line2D.Double(marker_begin_rechter_y_as, marker_eind_rechter_y_as);
            }
            ...
            if (marker_eind_rechter_y_as != null) {
              g2.drawString(
                  aanduiding,
                  (int) (marker_eind_rechter_y_as.getX() + afstand_aanduiding_naar_marker),
                  (int) (y_pos_marker + /*(stringHeight - leading) / 2*/ ascent_y / 2));
            }
          } // if ((k != 1) || (k == 1 && i % 3 == 0))
          ...
          if ((k != 1 && i != 0)
              || (k == 1
                  && i % 3 == 0
                  && i != 0)) // the x-axis (y = 0) itself do not over write and if wind dir plot
          // lines only every 30 degrees
          {
            g2.setStroke(new BasicStroke(2f));
        ...
        if (string_aanduiding.equals("") == false) {
          ...
          Rectangle2D bounds_string_aanduiding =
              font_sensor_level_text.getStringBounds(string_aanduiding, context_sensor_level_font);
          double ascent_sensor_level_text =
              -bounds_string_aanduiding
                  .getY(); // hoogte char's                      // ascent = helling/steilte (hoogte
          // characters)

          if (lb_grafiek != null) {
            // g2.drawString(string_aanduiding, (int)(lb_grafiek.getX()), (int)(2 *
            // ascent_sensor_level_text));
            g2.drawString(
                string_aanduiding,
                (int) (lb_grafiek.getX()),
                (int) (lb_grafiek.getY() - (0.5 * ascent_sensor_level_text)));
          }
        } // if (string_aanduiding.equals("") == false)
            ...
            if (!RS232_view.sensor_waarde_array[i].equals("")) {
              try {
                ...
                if ((RS232_view.mode_tijd_periode).equals(RS232_view.MODE_DAY)) {
                  if (lo_grafiek != null) {
                    x_waarde =
                        lo_grafiek.getX()
                            + (i
                                / (double) aantal_5_minuten_perioden_tussen_2_markers
                                * schaling_x_as);
                  }
                } else if ((RS232_view.mode_tijd_periode).equals(RS232_view.MODE_WEEK)) {
                  if (lo_grafiek != null) {
                ...
                if (lo_grafiek != null) {
                  y_waarde =
                      lo_grafiek.getY()
                          - ((sensor_waarde - parameter_start_waarde)
                              / (double) aantal_units_tussen_2_markers
                              * schaling);
                }
          ...
          for (int i = 0; i < RS232_view.AANTAL_PLOT_POINTS; i++) {
            ...
            if (!RS232_view.sensor_waarde_array_2[i].equals("")) {
              try {
                ...
                // NB North is 0 degr. from the sensors, this must be plotted as 360 degr for
                // consistency with the wind observations
            ...
            // if (RS232_view.sensor_waarde_array_d[i] != null)
            {
              try {
                ...
                if (main.wind_units.indexOf(main.M_S)
                    != -1) // wind_units 'as measured' set to m/s by observer in Maintenance ->
                // Station data
                {
                  if (main.wind_units_dashboard.indexOf(main.KNOTS)
                      != -1) // measured in m/s and dashboard in knots
                  {
                    sensor_waarde =
                        Double.parseDouble(RS232_view.sensor_waarde_array_d[i].trim())
                            * main.M_S_KNOT_CONVERSION;
                  } else if (main.wind_units_dashboard.indexOf(main.M_S)
                      != -1) // measured in m/s and dashboard in m/s
                  {
                    sensor_waarde = Double.parseDouble(RS232_view.sensor_waarde_array_d[i].trim());
                  }
                } else if (main.wind_units.indexOf(main.KNOTS)
                    != -1) // wind_units 'as measured' set to knots by observer in Maintenance ->
                // Station data
                {
                  if (main.wind_units_dashboard.indexOf(main.KNOTS)
                      != -1) // measured in knots and dashboard in knots
                  {
                    sensor_waarde = Double.parseDouble(RS232_view.sensor_waarde_array_d[i].trim());
                  } else if (main.wind_units_dashboard.indexOf(main.M_S)
                      != -1) // measured in knots and dashboard in m/s
                  {
                    sensor_waarde =
                        Double.parseDouble(RS232_view.sensor_waarde_array_d[i].trim())
          ...
          if (RS232_view.points[i].getX() != 0) {
            ...
            if (i + 1 < RS232_view.AANTAL_PLOT_POINTS) {
              if (RS232_view.points[i + 1].getX() != 0) {
                path.lineTo(
                    (float) RS232_view.points[i + 1].getX(),
                    (float)
                        RS232_view.points[i + 1]
                            .getY()); // Resets the path to empty. The append position is set back
                // to the beginning of the path and all coordinates and point
                // types are forgotten
              } else {
                g2.draw(path);
                path.reset();
        ...
        // write as a string the last parameter value (update every 5 minutes); top right corner
        // above graph
        //
        ...
        else if (k == 1) // graph b; top right; wind dir
        {
          for (int i = RS232_view.AANTAL_PLOT_POINTS - 1; i >= 0; i--) {
            if (!RS232_view.sensor_waarde_array_2[i].equals("")) {
              try {
                digitale_sensor_waarde =
                    Double.parseDouble(RS232_view.sensor_waarde_array_2[i].trim());
        ...
        else if (k == 2) // graph c; bottom left; air temp
        {
          for (int i = RS232_view.AANTAL_PLOT_POINTS - 1; i >= 0; i--) {
            if (!RS232_view.sensor_waarde_array_c[i].equals("")) {
              try {
                digitale_sensor_waarde =
                    Double.parseDouble(RS232_view.sensor_waarde_array_c[i].trim());
        ...
        if (k == 3) // graph d; bottom right; wind speed
        {
          ...
          // main.M_S_KNOT_CONVERSION) / 10.0d;
          ...
          // data
          {
            ...
            {
              digitale_sensor_waarde =
                  Math.round(digitale_sensor_waarde * 10 * main.M_S_KNOT_CONVERSION) / 10.0d;
            } else if (main.wind_units_dashboard.indexOf(main.M_S)
                != -1) // measured in m/s and dashboard in m/s
            {
          ...
          // data
          {
            ...
            {
              digitale_sensor_waarde =
                  Math.round(digitale_sensor_waarde * 10 * main.KNOT_M_S_CONVERSION) / 10.0d;
            }
          }
        } else {
          digitale_sensor_waarde =
              Math.round(digitale_sensor_waarde * 10) / 10.0d; // eg 998.19 -> 998.2
        }

        // write the string top right
        if ((digitale_sensor_waarde > -50)
            && (digitale_sensor_waarde
                < 1100)) // max/min of max limits of the parameters pressure, air temp, sst and wind
        // speed
        {
          ...
          {
            if (main.wind_units_dashboard.indexOf(main.KNOTS)
                != -1) // on graph (and dashboard) in knots
            {
              datum_tijd_parameter_string =
                  dag
                      + "-"
          ...
          Rectangle2D bounds_datum_tijd_pressure =
              font_datum_tijd_parameter.getStringBounds(
                  test_aanduiding_datum_tijd_parameter, context_datum_tijd_parameter);
          // double stringWidth_datum = bounds_datum.getWidth();
          double ascent_datum_tijd_pressure =
              -bounds_datum_tijd_pressure
                  .getY(); // hoogte char's                      // ascent = helling/steilte (hoogte
          // characters)
          double length_datum_tijd_pressure =
              bounds_datum_tijd_pressure.getWidth(); // lengte string

          // g2.drawString(datum_tijd_parameter_string, (int) (rechts_boven_grafiek.getX() -
          // length_datum_tijd_pressure), (int) (2 * ascent_datum_tijd_pressure));
          if (rb_grafiek != null) {
            g2.drawString(
                datum_tijd_parameter_string,
                (int) (rb_grafiek.getX() - length_datum_tijd_pressure),
                (int) (rb_grafiek.getY() - (0.5 * ascent_datum_tijd_pressure)));
          }
        } // if ((digitale_sensor_waarde > -50) && (digitale_sensor_waarde < 1100)) // max/min of
        // max limits of the parameters pressure, air temp, sst and wind speed
    ...
    else // 1 graph view
    {
      ...
      if (main.mode_grafiek.equals(main.MODE_PRESSURE)) {
        ...
        // color_pen    = color_pen_black;
          ...
          || main.mode_grafiek.equals(main.MODE_AIRTEMP_II)) {
        // Color color_raster_blue_air = new Color(114, 160, 193);          //  Air Superiority Blue
        // Color color_pen_red         = Color.RED;
        ...
        JOptionPane.showMessageDialog(
            null,
            "internal error 'mode_grafiek' in Function RS232_grafiek() unknown",
      ...
      // NB De hele achtergrond (bepaald door jPanel in design mode is nu floral white (255, 250,
      // 240))
      // NB onderstaande zou in deze jPanel de getekende rechthoek (kleiner dan jPanel) weer zwart
      // maken
      ...
      } else if (main.mode_grafiek.equals(
          main.MODE_WIND_SPEED)) // valid for true wind speed 10 min average and maximum true wind
      // gust from 10min
      {
        parameter_start_waarde = 0; // lowest parameter value in graph
        aantal_parameter_markers_y_as = 10; // actually 11 markers (if source included)
      ...
      //   links_onder_grafiek.getY() - {(990.6 - pressure_start_waarde) /
      // aantal_hpa_tussen_2_markers * schaling}
      ...
      if ((RS232_view.mode_tijd_periode).equals(RS232_view.MODE_DAY)) {
        ...
        aantal_time_markers_x_as =
            29; // 7 days every 6 hours (so 4 per dag), actually  30 markers if source also included
      }
      ...
      for (int i = 0; i <= aantal_time_markers_x_as; i++) {
        ...
        Point2D marker_begin_x_as =
            new Point2D.Double(x_pos_marker, links_onder_grafiek.getY() + lengte_x_as_mark);
        ...
        g2.drawString(
            uur_aanduiding,
            (int) (x_pos_marker - stringWidth_x / 2),
            (int) (links_onder_grafiek.getY() + lengte_x_as_mark + ascent_x));
        ...
        if (((RS232_view.mode_tijd_periode).equals(RS232_view.MODE_DAY)
          ...
          if (i
              != aantal_time_markers_x_as) // niet bij de allerlaaste x-as marker (komt dan voorbij
          // de rechter y-as)
          {
            // eg Thuesday
            g2.drawString(
                dag_van_de_week_aanduiding,
                (int) (x_pos_marker),
                (int) (links_onder_grafiek.getY() + lengte_x_as_mark + (2 * ascent_datum)));

            // eg 12-05-2015
            g2.drawString(
                datum_aanduiding,
                (int) (x_pos_marker),
                (int) (links_onder_grafiek.getY() + lengte_x_as_mark + (3 * ascent_datum)));
          }
        ...
        if (i != 0) // the y-axis (x = 0) itself do not over write
        {
          ...
          // g2.setPaint(Color.LIGHT_GRAY);
          Point2D verticaal_hulp_lijn_begin =
              new Point2D.Double(x_pos_marker, links_onder_grafiek.getY());
          Point2D verticaal_hulp_lijn_eind =
              new Point2D.Double(x_pos_marker, links_boven_grafiek.getY());
      ...
      for (int i = 0; i <= aantal_parameter_markers_y_as; i++) {
        ...
        Point2D marker_begin_linker_y_as =
            new Point2D.Double(links_onder_grafiek.getX() - lengte_y_as_mark, y_pos_marker);
        Point2D marker_eind_linker_y_as =
            new Point2D.Double(links_onder_grafiek.getX(), y_pos_marker);

        Line2D line_marker_linker_y_as =
            new Line2D.Double(marker_begin_linker_y_as, marker_eind_linker_y_as);
        g2.draw(line_marker_linker_y_as);
        ...
        g2.drawString(
            aanduiding,
            (int)
                (marker_begin_linker_y_as.getX() - stringWidth_y - afstand_aanduiding_naar_marker),
            (int) (y_pos_marker + ascent_y / 2));
        ...
        Point2D marker_begin_rechter_y_as =
            new Point2D.Double(rechts_onder_grafiek.getX(), y_pos_marker);
        Point2D marker_eind_rechter_y_as =
            new Point2D.Double(rechts_onder_grafiek.getX() + lengte_y_as_mark, y_pos_marker);

        Line2D line_marker_rechter_y_as =
            new Line2D.Double(marker_begin_rechter_y_as, marker_eind_rechter_y_as);
        g2.draw(line_marker_rechter_y_as);
        ...
        if (i != 0) // the x-axis (y = 0) itself do not over write
        {
          ...
          Point2D horizontaal_hulp_lijn_begin =
              new Point2D.Double(links_onder_grafiek.getX(), y_pos_marker);
          Point2D horizontaal_hulp_lijn_eind =
              new Point2D.Double(rechts_onder_grafiek.getX(), y_pos_marker);

          Line2D hulp_lijn =
              new Line2D.Double(horizontaal_hulp_lijn_begin, horizontaal_hulp_lijn_eind);
          g2.draw(hulp_lijn);
        } // if (i != 0)
        ...
        if (i != aantal_parameter_markers_y_as) {
          ...
          for (int j = 0; j < aantal_secondary_lines; j++) {
            double y_pos_sub_marker =
                (links_onder_grafiek.getY() - (i * schaling))
                    - (j * (schaling / aantal_units_tussen_2_markers));
            Point2D horizontaal_hulp_lijn_begin =
                new Point2D.Double(links_onder_grafiek.getX(), y_pos_sub_marker);
            Point2D horizontaal_hulp_lijn_eind =
                new Point2D.Double(rechts_onder_grafiek.getX(), y_pos_sub_marker);
      ...
      if (main.RS232_connection_mode == 1
          ...
          || main.RS232_connection_mode
              == 6) // PTB220 or PTB330 or MintakaDuo or Mintaka Star (USB) Mintaka Star WiFi
      {
        if (main.mode_grafiek.equals(main.MODE_PRESSURE)) {
        ...
        // Rectangle2D bounds_string_aanduiding =
        // font_sensor_level_text.getStringBounds(string_aanduiding, context_sensor_level_font);
        // double ascent_sensor_level_text = -bounds_string_aanduiding.getY();     // hoogte char's
        //                     // ascent = helling/steilte (hoogte characters)
        // g2.drawString(string_aanduiding, (int)(links_boven_grafiek.getX()), (int)(2 *
        // ascent_sensor_level_text));
      } else if (main.RS232_connection_mode == 7
          || main.RS232_connection_mode == 8) // StarX USB or StarX LAN connected
      {
        if (main.mode_grafiek.equals(main.MODE_PRESSURE)) {
          // string_aanduiding = "All air pressure values at Mean Sea Level";          // till
          // 22-06-2018
      ...
      if (string_aanduiding.equals("") == false) {
        ...
        g2.drawString(
            string_aanduiding,
            (int) (links_boven_grafiek.getX()),
            (int) (2 * ascent_sensor_level_text));
      }
          ...
          try {
            ...
            if (main.mode_grafiek.equals(main.MODE_WIND_SPEED)) {
              ...
              // Station data
              {
                ...
                {
                  sensor_waarde =
                      Double.parseDouble(hulp_sensor_waarde_array[i].trim())
                          * main.M_S_KNOT_CONVERSION;
                } else if (main.wind_units_dashboard.indexOf(main.M_S)
                    != -1) // measured in m/s and dashboard in m/s
                {
            ...
            if ((RS232_view.mode_tijd_periode).equals(RS232_view.MODE_DAY)) {
              x_waarde =
                  links_onder_grafiek.getX()
                      + (i / (double) aantal_5_minuten_perioden_tussen_2_markers * schaling_x_as);
            } else if ((RS232_view.mode_tijd_periode).equals(RS232_view.MODE_WEEK)) {
              x_waarde =
                  links_onder_grafiek.getX()
                      + (i / (double) aantal_5_minuten_perioden_tussen_2_markers * schaling_x_as);
          ...
          if (i + 1 < RS232_view.AANTAL_PLOT_POINTS) {
            if (RS232_view.points[i + 1].getX() != 0) {
              path.lineTo(
                  ...
                  (float)
                      RS232_view.points[i + 1]
                          .getY()); // Resets the path to empty. The append position is set back to
              // the beginning of the path and all coordinates and point types
              // are forgotten
            } else {
              g2.draw(path);
              path.reset();
      ...
      // write as a string the last parameter value (update every 5 minutes); top right corner above
      // graph
      //
      for (int i = RS232_view.AANTAL_PLOT_POINTS - 1; i >= 0; i--) {
        if (!hulp_sensor_waarde_array[i].equals("")) {
          try {
            ...
            if ((digitale_sensor_waarde > -50)
                && (digitale_sensor_waarde
                    < 1100)) // max/min of max limits of the parameters pressure, air temp, sst and
            // wind speed
            {
              ...
              double ascent_datum_tijd_pressure =
                  -bounds_datum_tijd_pressure
                      .getY(); // hoogte char's                      // ascent = helling/steilte
              // (hoogte characters)
              double length_datum_tijd_pressure =
                  bounds_datum_tijd_pressure.getWidth(); // lengte string
              ...
              g2.drawString(
                  datum_tijd_parameter_string,
                  (int) (rechts_boven_grafiek.getX() - length_datum_tijd_pressure),
                  (int) (2 * ascent_datum_tijd_pressure));
            }
          } // try
          catch (NumberFormatException e) {
          }
      ...
      if (main.mode_grafiek.equals(main.MODE_WIND_SPEED)) {
        ...
        //
        // public BasicStroke(float width, int cap, int join, float miterlimit, float[] dash, float
        // dash_phase)
        //
        // The dash pattern of a line is actually controlled by two attributes: the dash array and
        // the dash phase.
        // The dash array is a float[] that specifies the number of units to be drawn followed by
        // the number of units
        // to be skipped. For example, to draw a dashed line in which both the dashes and spaces are
        // 25 units long,
        ...
        //
        //
        // If, for some reason, you want to draw a dashed line but do not want your line to begin at
        // the
        // beginning of the dash pattern, you can specify the dash-phase attribute. The value of
        // this
        // attribute specifies how far into the dash pattern the line should begin. Note, however,
        // that this value is not an integer index into the dash pattern array. Instead, it is a
        // floating-point
        ...
        // BasicStroke bs_dashed = new BasicStroke(1.25f, BasicStroke.CAP_BUTT,
        // BasicStroke.JOIN_ROUND, 10.0f, dash, 2.0f);

        // BasicStroke bs_dashed = new BasicStroke(1.25f, BasicStroke.CAP_BUTT,
        // BasicStroke.JOIN_ROUND, 10.0f, new float[]{100f,200f}, 2.0f);
