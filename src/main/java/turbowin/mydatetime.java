...
public final class mydatetime extends javax.swing.JFrame {
  ...
  private void Internet_button_actionPerformed(
    ...
    main.help_mouseClicked(DATETIME_HELP_DIR);
  ...
  private void OK_button_actionPerformed(
    ...
    if (main.RS232_connection_mode != 3) // not AWS connected mode
    {
      ...
      if (checks_ok == true) {
        ...
        int iDay =
            1; // only for determining max number of days in a particular month -> 1 is always ok
        // for testing (filled in day e.g. 31 could be misleading!)
        ...
        if (daysInMonth
            < Integer.parseInt(
                day)) // daysInMonth = max possible in that month; day = as inserted by the user
        {
          JOptionPane.showMessageDialog(
              null,
              "combination month and day of month not valid",
              main.APPLICATION_NAME + " error",
              JOptionPane.WARNING_MESSAGE);
          Reset_All_DateTime_Vars();
        } // if (daysInMonth < day)
      ...
      if (checks_ok == true) {
        // NB String variable year can directly used in IMMT (So no conversion like the month var
        // necessary)
  ...
  private void initSynopparameters() {
    ...
    if (year.compareTo("") != 0) jList1.setSelectedValue(year, true);

    if (month.compareTo("") != 0) jList2.setSelectedValue(month, true);
  ...
  // constants
  private final String DATETIME_HELP_DIR = "date_time.pdf";
  ...
  public static String minute =
      "00"; // 00 default for manual obs but overwritten by AWS (SMD output data displayed on
  // TurboWin+ main screen)
