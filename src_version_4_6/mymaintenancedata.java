         ...
         if (doorgaan == true)
         {
            new SwingWorker<String, Void>()
            {
               ...
               protected String doInBackground() throws Exception
               {
                  ...
                  try (BufferedWriter out = new BufferedWriter(new FileWriter(maintenance_data_export_file)))
                  {
                     for (int i = 0; i < main.MAX_AANTAL_CONFIGURATIEREGELS; i++)
                     {
                        if ((main.configuratie_regels[i] != null) && (main.configuratie_regels[i].compareTo("") != 0))
                  ...
                  {
                     export_maintenance_data_ok = "Unable to write to: " + maintenance_data_export_file + " (" + ex + ")";
                  } // catch
      ...
      if (result == JFileChooser.APPROVE_OPTION)
      {
         ...
         new SwingWorker<String, Void>()
         {
            ...
            protected String doInBackground() throws Exception
            {
               ...
               try (BufferedReader in = new BufferedReader(new FileReader(maintenance_data_import_file)))
               {
                  teller = 0;
               ...
               if (ship_name_txt_present && call_sign_txt_present)
               {
                  ...
                  // reset reading!!
                  try (BufferedReader in2 = new BufferedReader(new FileReader(maintenance_data_import_file)))
                  {   
                     teller = 0;
