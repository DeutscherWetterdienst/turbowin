...
public void paintComponent(Graphics g)
{
   ...
   //
   if ( (DASHBOARD_view.night_vision == false) && (main.dashboard_background_image.compareTo("") != 0) )
   {
      try
      {
         Image img_background = new ImageIcon(main.dashboard_background_image).getImage();
         g2d.drawImage(img_background, 0, 0, (getWidth()), (getHeight()), 0, 0, img_background.getWidth(null), img_background.getHeight(null), null);
         
         // NB "main.dashboard_background_image": saved in Function choose_dashboard_image_barometer() [DASHBOARD_view_AWS_radar.java]
      }
      catch (Exception e)
      {
   ...
   {
      // background photo (only in GUI_SOT mode and not in night vision mode and not in transparent mode)
      //
      if ( (DASHBOARD_view.night_vision == false) && (!main.theme_mode.equals(main.THEME_TRANSPARENT)) )
      {
         ...
         //  NB the line below (Image img1 = ... etc.)  didn't work (image file not found) anymore from 2022 why   ???????
         //
