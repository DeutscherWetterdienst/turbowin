...
public class ObservationFillingTest extends AbstractUiTest {
  ...
  private void closeStartDialogs() throws InterruptedException {
    DialogFixture turboWinWarningFrame =
        WindowFinder.findDialog(new DialogTitleMatcher("TurboWin+ warning")).using(this.robot());
    TimeUnit.MILLISECONDS.sleep(TIMEOUT);
    turboWinWarningFrame
        .button(JButtonMatcher.withText("OK"))
        .requireVisible()
        .requireEnabled()
        .click();
    TimeUnit.MILLISECONDS.sleep(TIMEOUT);
    DialogFixture dateTimeDialog =
        WindowFinder.findDialog(new DialogTitleMatcher("Date and Time")).using(this.robot());
    TimeUnit.MILLISECONDS.sleep(TIMEOUT);
  ...
  private void fillPositionInformation() throws InterruptedException {
    ...
    FrameFixture positionFrame =
        WindowFinder.findFrame(new FrameTitleMatcher("Position, course and speed"))
            .using(this.robot());
  ...
  private void handleLocalizedButtonClick(DialogFixture dialog, String textEn, String textDe) {
    try {
      ...
      try {
        ...
        System.out.println(
            "Error finding button: " + e.getMessage() + "\nsecond Error: " + ex.getMessage());
  ...
  private void fillDateTimeInformation() throws InterruptedException {
    this.frame
        .label(JLabelMatcher.withText("Date & Time obs"))
        .requireVisible()
        .requireEnabled()
        .click();
    TimeUnit.MILLISECONDS.sleep(TIMEOUT);
    ...
    FrameFixture dateTimeFrame =
        WindowFinder.findFrame(new FrameTitleMatcher("Date and Time")).using(this.robot());
  ...
  private void fillPressureInformation() throws InterruptedException {
    this.frame
        .label(JLabelMatcher.withText("Pressure (read+ic)"))
        .requireVisible()
        .requireEnabled()
        .click();
    FrameFixture pressureFrame =
        WindowFinder.findFrame(new FrameTitleMatcher("barometer reading")).using(this.robot());
  ...
  private void fillPressureTendencyInformation() throws InterruptedException {
    this.frame
        .label(JLabelMatcher.withText("Pressure tendency"))
        .requireVisible()
        .requireEnabled()
        .click();
    FrameFixture pressureTendencyFrame =
        WindowFinder.findFrame(new FrameTitleMatcher("barograph reading")).using(this.robot());
  ...
  private void fillAirTemperatureInformation() throws InterruptedException {
    ...
    FrameFixture temperaturesFrame =
        WindowFinder.findFrame(new FrameTitleMatcher("Temperatures")).using(this.robot());
  ...
  private void fillTrueWindInformation() throws InterruptedException {
    ...
    FrameFixture windFrame =
        WindowFinder.findFrame(new FrameTitleMatcher("Wind")).using(this.robot());
  ...
  private void fillWaveInformation() throws InterruptedException {
    this.frame
        .label(JLabelMatcher.withText("(Wind) wave per"))
        .requireVisible()
        .requireEnabled()
        .click();
    FrameFixture wavesFrame =
        WindowFinder.findFrame(new FrameTitleMatcher("Waves")).using(this.robot());
  ...
  private void fillVisibilityInformation() throws InterruptedException {
    this.frame
        .label(JLabelMatcher.withText("Visibility"))
        .requireVisible()
        .requireEnabled()
        .click();
    FrameFixture visibilityFrame =
        WindowFinder.findFrame(new FrameTitleMatcher("Visibility")).using(this.robot());
  ...
  private void fillPresentWeatherInformation() throws InterruptedException {
    this.frame
        .label(JLabelMatcher.withText("Present weath."))
        .requireVisible()
        .requireEnabled()
        .click();
    FrameFixture visibilityFrame =
        WindowFinder.findFrame(new FrameTitleMatcher("Present weather")).using(this.robot());
  ...
  private void fillPastWeatherInformation() throws InterruptedException {
    this.frame
        .label(JLabelMatcher.withText("Past weath. 1st"))
        .requireVisible()
        .requireEnabled()
        .click();
    FrameFixture pastWeatherFrame =
        WindowFinder.findFrame(new FrameTitleMatcher("Past weather")).using(this.robot());
  ...
  private void fillLowCloudInformation() throws InterruptedException {
    ...
    FrameFixture lowCloudsFrame =
        WindowFinder.findFrame(new FrameTitleMatcher("Clouds low")).using(this.robot());
  ...
  private void fillMiddleCloudInformation() throws InterruptedException {
    ...
    FrameFixture lowCloudsFrame =
        WindowFinder.findFrame(new FrameTitleMatcher("Clouds middle")).using(this.robot());
  ...
  private void fillHighCloudInformation() throws InterruptedException {
    ...
    FrameFixture lowCloudsFrame =
        WindowFinder.findFrame(new FrameTitleMatcher("Clouds high")).using(this.robot());
  ...
  private void fillTotalCloudCoverInformation() throws InterruptedException {
    this.frame
        .label(JLabelMatcher.withText("Total cloud cov"))
        .requireVisible()
        .requireEnabled()
        .click();
    FrameFixture lowCloudsFrame =
        WindowFinder.findFrame(
                new FrameTitleMatcher(
                    "Total cloud cover, amount of Cl (Cm) and height of base of lowest cloud"))
            .using(this.robot());
  ...
  private void addObserverInformation() throws InterruptedException {
    ...
    FrameFixture observerFrame =
        WindowFinder.findFrame(new FrameTitleMatcher("Observer")).using(this.robot());
  ...
  private void closeRestartDialog() throws InterruptedException {
    DialogFixture turboWinInfoFrame =
        WindowFinder.findDialog(new DialogTitleMatcher("TurboWin+ info")).using(this.robot());
  ...
  private void clickOnStationData() {
    ...
    JMenuItemFixture maintenanceStationDataMenu =
        this.frame.menuItemWithPath("Maintenance|Station data...");
  ...
  private void enterStationData() throws InterruptedException {
    ...
    FrameFixture stationDataFrame =
        WindowFinder.findFrame(new FrameTitleMatcher("Station data")).using(this.robot());
    ...
    stationDataFrame
        .radioButton(new RadioButtonMatcher("estimated; true speed and true direction"))
        .click();
  ...
  private void enterPassword() throws InterruptedException {
    try {
      FrameFixture passwordFrame =
          WindowFinder.findFrame(new FrameTitleMatcher("Password"))
              .withTimeout(TIMEOUT)
              .using(this.robot());
    ...
    FrameFixture passwordFrame =
        WindowFinder.findFrame(new FrameTitleMatcher("Password")).using(this.robot());
  ...
  private void verifyFM13Creation() throws InterruptedException {
    ...
    assertEquals(
        "BBXX 00000 29103 99101 10122 41494 40105 10050 20050 49915 51050 74765 83246 22212 00050 20510 80050=",
        fm13Field.text());
  }
