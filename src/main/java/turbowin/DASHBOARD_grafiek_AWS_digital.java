...
public class DASHBOARD_grafiek_AWS_digital extends JPanel {
  ...
  public void paintComponent(Graphics g) {
    ...
    // width of a digital instrument (6 spaces + 5 instruments = 11 blocks; instument width and
    // space width are equal here)
    // NB calculating instrument_width can't be done in DASHBOARD_grafiek_AWS_digital() because from
    ...
    //
    instrument_width =
        DASHBOARD_view_AWS_digital.width_AWS_digital_dashboard
            / 6; // NB 6 = 5 instument rows + some space
    instrument_height = instrument_width / 2;
    instrument_ring_thickness = instrument_width / 10;
    horz_offset =
        (DASHBOARD_view_AWS_digital.width_AWS_digital_dashboard - (5 * instrument_width))
            / 6; // 6 space blocks in one row of 5 instruments

    fontSize_b =
        (int)
            (instrument_height
                - (2 * instrument_ring_thickness)
                - 3); // measured value  (eg 1002.3)               // -5 for just a slightly better
    // fit
    ...
    try {
      ...
      // Font font_digital = Font.createFont(Font.TRUETYPE_FONT, new File(main.logs_dir +
      // java.io.File.separator + DIGITAL_FONT_DIR + java.io.File.separator + digital_font));
      Font font_digital =
          Font.createFont(
              Font.TRUETYPE_FONT,
              this.getClass().getResourceAsStream(main.ICONS_DIRECTORY + digital_font));
      font_b =
          font_digital.deriveFont(Font.BOLD, (float) fontSize_b); // measured value  (eg 1002.3)
      font_d =
          font_digital.deriveFont(
              Font.BOLD, (float) fontSize_d); // parameter text (eg "Temp [deg]")
      font_e =
          font_digital.deriveFont(
              Font.BOLD, (float) fontSize_e); // last updated text in middle screen
    ...
    */
    ...
    g2d.fill(
        new RoundRectangle2D.Double(
            x_left_label, y_top_label, x_right_label, y_bottom_label, 20, 20));
    ...
    draw_AWS_digital_instrument(
        g2d, low_limit, max_limit, label_instrument, reading_present, reading);
    ...
    if (main.VOT_from_AWS_present) {
      if (main_RS232_RS422.dashboard_int_last_update_record_VOT >= 0
        ...
        DASHBOARD_view_AWS_digital.jButton1.setEnabled(
            true); // button: "make an visual observation"
      ...
      if (main.AWSR) // e.g. OMC-140 but NOT EUCAWS or APR
      {
        ...
        if (((AWSR_system_hour + 1) % Integer.valueOf(main.AWSR_reporting_interval)) == 0) {
          ...
          if (AWSR_minutes_for_upload <= 30) {
            DASHBOARD_view_AWS_digital.jButton1.setEnabled(true);
          } else {
            DASHBOARD_view_AWS_digital.jButton1.setEnabled(false);
          }
        } else {
  ...
  private void draw_AWS_digital_instrument(
    ...
    // https://www.google.nl/search?q=barometer&source=lnms&tbm=isch&sa=X&ved=0ahUKEwjJ78PZg8_UAhWEJMAKHZA3AsAQ_AUICigB&biw=1920&bih=950#imgrc=hE2asDauQrhr6M:
    double
        reading_abs; // needed for displaying negative values (the "-" sign is not part of the lcd
    // font set)
    ...
    if (reading_present) {
      ...
      if (reading >= low_limit && reading <= max_limit) {
        reading_abs =
            Math.abs(
                reading); // the "-" sign is not correct displayed in de LCD font set (need a trick)

        String digits = "";

        // wind dir and speed displayed as eg 350 and not 350.0 (wind dir and wind speed and gust
        ...
        // int y_digits = -(int)instrument_height / 2 + (height_b1 / 2) -
        // (int)instrument_ring_thickness;
