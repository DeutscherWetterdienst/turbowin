...
public class Obs_Stats_graph extends JPanel {
  ...
  public void paintComponent(Graphics g) {
    ...
    // NB var 'immt_log_ok' will be set in Obs_Stats_view.java in a swingworker thread so possible
    // the boolean value is set later
    //    so it could be that the graph with the first 50 observers bars is diplayed with the
    // warning pop-up message
    //    (" > 50 different observer names recorded in immt.log. Please send logs to your Port
    // Meteorological Officer")
    //    but it is also possible the pop-up message is displayed over an "empty" graph, all depends
    // on when the swingworker thread is finished
    //    but anyhow, both scenario's are ok and do not cause an exception
    //
    if (Obs_Stats_view
        .immt_log_ok) // NB var immt_log_ok will be set in Obs_Stats_view.java in a swingworker
    // thread so possible the boolean value is
    {
      ...
      Point2D links_boven_grafiek =
          new Point2D.Double(linker_kantlijn_margin, boven_kantlijn_margin);
      ...
      if (main.obs_stats_mode.equals(main.OBSERVERS_STATS)) {
        aantal_units_tussen_2_markers =
            10; // nb units = number obs or hPa or degrees or wind speed etc.
      } else if (main.obs_stats_mode.equals(main.OBSERVATIONS_STATS)) {
        aantal_units_tussen_2_markers =
            100; // nb units = number obs or hPa or degrees or wind speed etc.
      }
      ...
      //
      // read IMMT log and draw the bars with observers count or with the different obs parameters
      // count
      //
      if (Obs_Stats_view.immt_log_ok) {
        if (main.obs_stats_mode.equals(main.OBSERVERS_STATS)) {
          draw_Observers_Bars(
              g2,
              links_onder_grafiek,
              rechts_onder_grafiek,
              schaling,
              aantal_units_tussen_2_markers,
              max_obs);
        } else if (main.obs_stats_mode.equals(main.OBSERVATIONS_STATS)) {
          draw_Observations_Bars(
              g2,
              links_onder_grafiek,
              rechts_onder_grafiek,
              schaling,
  ...
  private void draw_Observations_Bars(
      ...
      Point2D right_below_graph,
      double scaling,
      int number_units_between_2_markers,
      int max_obs) {
    // NB lay out scheme
    ...
    Rectangle2D bounds_parameter_name =
        font_parameter_name.getStringBounds(test_aanduiding_y, context_parameter_name);
    ...
    for (String obs : Obs_Stats_view.immt_list) {
      if (obs.length() >= main.IMMT_5_LENGTH) // avoiding out of index-range exceptions
      {
        ...
        if ((height_clouds.indexOf(' ') == -1) && (height_clouds.indexOf('/') == -1)
            ...
            || (amount_lowest_clouds.indexOf(' ') == -1)
                && (amount_lowest_clouds.indexOf('/') == -1)
        ...
        if ((ice_accretion.indexOf(' ') == -1) && (ice_accretion.indexOf('/') == -1)
            || (thickness_ice_accretion.indexOf(' ') == -1)
                && (thickness_ice_accretion.indexOf('/') == -1)
    ...
    // e.g. 6 different observer names -> 7 (6+1) intermediate spaces between the bars -> 13
    // 'blocks'(6 bars and 7 spaces)
    //
    //
    //
    // | -----------------------------------------------------------------------------|   // x-as
    ...
    // | <--->,<--->,<--->,<--->,<--->,<--->,<--->,<--->,<--->,<---><--->,<--->,<---> |   // 13
    // blocks
    // | space  bar  space  bar  space  bar  space  bar  space  bar space  bar  space |   // 6 bars
    // + 7 paces = 13 blocks
    //
    //
    //
    ...
    for (int i = 0; i < number_blocks; i++) {
      ...
      if ((i % 2) != 0) // odd (bar with number of observations)
      {
        ...
        int number_obs_display =
            number_obs; // to display the real number on top of the bar (and not the rounded to 100
        // number if real obs number > 100)
        ...
        g2d.fillRect(
            (int) left_below_graph.getX() + x_bar, y_bar - bar_height, (int) bar_width, bar_height);

        // number of obs on top of the bar (on the left, to avoid text cluttering with the parameter
        // name)
        if (number_obs_display > 0) {
          ...
          g2d.drawString(
              Integer.toString(number_obs_display), x_string_number_obs, y_string_number_obs);
        }

        // name of the parameter (e.g. air pressure)
        int x_string =
            (int)
                (left_below_graph.getX()
                    + x_bar
                    + (bar_width / 2)
                    - stringWidth_parameter_name / 2);
        ...
        x_string =
            (int) (left_below_graph.getX() + x_bar + (bar_width / 2) + (ascent_parameter_name / 2));
    ...
    if (!Obs_Stats_view.immt_list.isEmpty()) {
      if (Obs_Stats_view.view_immt_log_period.equals(myimmtlogperiod.CUSTOM)) {
        DateTimeFormatter dateTimeFormatter =
            DateTimeFormatter.ofPattern("dd-MMM-yyyy"); // e.g. 21-Sep-2025
      ...
      String log_line_1 =
          Obs_Stats_view
              .immt_rec_first; // always the first line of the complete raw available immt log
      String log_line_last =
          Obs_Stats_view
              .immt_rec_last; // always the last line of the complete raw available immt log
  ...
  private void draw_Observers_Bars(
      ...
      int number_units_between_2_markers,
      int max_obs) {
    // NB only the saved observations with a observer name at the end of the record string will be
    // taken into account
    ...
    Rectangle2D bounds_observer_name =
        font_observer_name.getStringBounds(test_aanduiding_y, context_observer_name);
    double stringWidth_observer_name = bounds_observer_name.getWidth();
    double ascent_observer_name =
        -bounds_observer_name
            .getY(); // The ascent is the distance from the baseline to the top of the font
    ...
    for (String obs : Obs_Stats_view.immt_list) {
          ...
          > main.IMMT_5_POSITION_OBSERVER - 1) // main.IMMT_5_POSITION_OBSERVER - 1 = 172
      {
        observer_name =
            obs.substring(
                main.IMMT_5_POSITION_OBSERVER); // main.IMMT_5_POSITION_OBSERVER = 173  // e.g.
        // Janssen;P;4 off;-;
        ...
        if (!observer_name.equals("")) {
          ...
          if (pos_1 != -1 && pos_2 != -1) {
            observer_name =
                observer_name.substring(0, pos_1)
                    + " "
                    + observer_name.substring(pos_1 + 1, pos_2); // e.g. Janssen P
          }
        } // if (!observer_name.equals(""))
    ...
    for (int i = 0; i < number_blocks; i++) {
      ...
      if ((i % 2) != 0) // odd (bar with number of observations)
      {
        ...
        int x_string =
            (int)
                (left_below_graph.getX() + x_bar + (bar_width / 2) - stringWidth_observer_name / 2);
        int y_string = (int) left_below_graph.getY();
        ...
        if (different_observer_names <= 2) {
          ...
          g2d.drawString(
              observers_names_array[i - pass], x_string, y_string + (int) (ascent_observer_name));
        } else {
          // set the observer name vertical in the bar
          x_string =
              (int)
                  (left_below_graph.getX() + x_bar + (bar_width / 2) + (ascent_observer_name / 2));
  ...
  private void write_period_log_line(
      ...
      final String requested_start_date,
      final String requested_end_date) {
    String text_1 = "";
    String text_2 = "";

    // log line e.g. "displayed log period: 01-Jan-2022 / 29-Nov-2023 (available log period:
    // 12-Sep-2023 / 10-Dec-2023)"
    //          or  "log period: 12-Sep-2023 / 10-Dec-2023"

    // custom requested log period
    //
    if ((Obs_Stats_view.view_immt_log_period.equals(myimmtlogperiod.CUSTOM))
        && (!requested_start_date.equals(""))
        && (!requested_end_date.equals(""))) {
    ...
    //
    // see IMMT description
    // e.g. IMMT record: 32018051112152700620         0129                     0
    // 44TESTNL US 314            23           A599999199999999991191                    9999 999
    //  01234567
    ...
    // NB String strDate_1 = day_1 + "-" + month_1_let + "-" + year_1 + " " + hour_1 + ":00 UTC";
    // // from verion 4.6: commentent out, with the time the log line could be very long
    ...
    // NB String strDate_last = day_last + "-" + month_last_let + "-" + year_last + " " + hour_last
    // + ":00 UTC"; // from verion 4.6: comment out,with the time the log line could be very long
