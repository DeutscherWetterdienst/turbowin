...
public class Python_Email 
{
   ...
   private static boolean isDebianBased()            // NB = also Ubuntu based
   {
      ...
      if (osInfo != null) 
      {
         ...
         if (id.contains("debian") || idLike.contains("debian"))  // idLike.contains("debian"): Catches derivatives like Ubuntu, Linux Mint, etc.
         {
            return true;
         }
      }
   ...
   private static boolean isRedHatBased() 
   {
      ...
      if (osInfo != null) 
      {
         String id = osInfo.getOrDefault("ID", "").toLowerCase();
         String idLike = osInfo.getOrDefault("ID_LIKE", "").toLowerCase();
         if (id.contains("rhel") || id.contains("fedora") || id.contains("centos") || idLike.contains("rhel")) 
         {
            return true;
         }
      }
   ...
   public final String python_email_exe_return_status_to_text(int exit_status)
   {
      ...
      switch (exit_status)
      {
         ...
         case 10:  exit_status_text = "SMTP authentication went wrong. Most probably the server did not accept the username/password combination provided"; break;
         ...
         case 15:  exit_status_text = "Error, raised to signal an error from the underlying SSL implementation."; break;
         case 16:  exit_status_text = "Error, raised when trying to read or write and the SSL connection has been closed cleanly. Note that this does not mean that the underlying transport (read TCP) has been closed."; break;
         case 17:  exit_status_text = "Error, raised by a non-blocking SSL socket when trying to read or write data, but more data needs to be received on the underlying TCP transport before the request can be fulfilled."; break;
         case 18:  exit_status_text = "Error, raised by a non-blocking SSL socket when trying to read or write data, but more data needs to be received on the underlying TCP transport before the request can be fulfilled."; break;
         case 19:  exit_status_text = "Error, raised by a non-blocking SSL socket when trying to read or write data, but more data needs to be sent on the underlying TCP transport before the request can be fulfilled."; break;
         case 20:  exit_status_text = "Error, raised when a system error was encountered while trying to fulfill an operation on a SSL socket. Unfortunately, there is no easy way to inspect the original errno number."; break;
         case 21:  exit_status_text = "Error, raised when the SSL connection has been terminated abruptly. Generally, you should not try to reuse the underlying transport when this error is encountered."; break;
         case 22:  exit_status_text = "Error, raised when certificate validation has failed."; break;
