...
public class Python_Email {
      ...
      try (BufferedReader reader = new BufferedReader(new FileReader(osReleaseFile))) {
        ...
        while ((line = reader.readLine()) != null) {
          if (line.contains("=")) {
            ...
            osInfo.put(key, value);
          }
        }

        return osInfo;
      } catch (IOException e) {
        // System.err.println("Error reading /etc/os-release: " + e.getMessage());
        osInfo = null;
      }
    }
  ...
  private static boolean isDebianBased() // NB = also Ubuntu based
    ...
    // Modern method: Check if "ID=debian" or "ID_LIKE=debian" in /etc/os-release
    Map<String, String> osInfo = parseOsRelease();
    if (osInfo != null) {
      String id = osInfo.getOrDefault("ID", "").toLowerCase();
      String idLike = osInfo.getOrDefault("ID_LIKE", "").toLowerCase();
      if (id.contains("debian")
          || idLike.contains(
              "debian")) // idLike.contains("debian"): Catches derivatives like Ubuntu, Linux Mint,
      // etc.
      {
        return true;
      }
    ...
    // Legacy method: Check for /etc/debian_version
  ...
  private static boolean isRedHatBased() {
    ...
    // Modern method: Check "ID" or "ID_LIKE" in /etc/os-release
    Map<String, String> osInfo = parseOsRelease();
    if (osInfo != null) {
      String id = osInfo.getOrDefault("ID", "").toLowerCase();
      String idLike = osInfo.getOrDefault("ID_LIKE", "").toLowerCase();
    ...
    // Legacy method: Check for /etc/redhat-release
  ...
  public boolean python_email_control_center() {
    ...
    switch (ostype) {
      case LINUX: // python_email_exe   = PYTHON_EMAIL_EXE_LIN;
        // break;
        if (isDebianBased()) {
          python_email_exe = PYTHON_EMAIL_EXE_LIN_DEB;
        } else if (isRedHatBased()) {
          python_email_exe = PYTHON_EMAIL_EXE_LIN_RED;
        } else {
          // this is a guess
          python_email_exe = PYTHON_EMAIL_EXE_LIN_DEB;
        }
        break;
      case WINDOWS:
        python_email_exe = PYTHON_EMAIL_EXE_WIN;
        break;
      default:
        python_email_exe = PYTHON_EMAIL_EXE_WIN;
        break;
    ...
    if (return_status != 0) // python_email_exe not found so now try to copy python_email_exe
    {
      ...
      if (return_status
          != 0) // now if return_status = 0 python_email_exe was copied sucessfully from jar to
      // "PYTHON_ROOT_DIR"
      {
        doorgaan = false;
      }
  ...
  private int check_python_email_module() {
    ...
    int python_email_module_status =
        0; // NB 0 = python_email_exe is present; 1 = python_email_exe is missing

    if (main.logs_dir.trim().equals("") == false && main.logs_dir.trim().length() >= 2) {
      // NB see function copy_python_email_exe_module() for a message if "PYTHON_ROOT_DIR" do not
      // exist

      // NB e.g. python_email_exe_File = "C:\Program Files
      // (x86)\TurboWin+\logs\python\email_tbw.exe"
      final File python_email_exe_File =
          new File(
              main.logs_dir
  ...
  private int copy_python_email_module() {
    ...
    if (doorgaan == true) {
      main.log_turbowin_system_message(
          "[EMAIL] start copying complete Python Email module from jar to: " + main.logs_dir);
      ...
      if (getClass().getResourceAsStream(resource) != null) {
        try (InputStream is =
          ...
          main.log_turbowin_system_message(
              "[EMAIL] success when copying " + resource + " from jar to: " + python_email_dir);
          ...
          switch (ostype) {
            case LINUX:
              ...
              if (file.exists()) {
                ...
                boolean result =
                    file.setExecutable(true, false); // Setting execute permission for all

                // System.out.println("Is execute permission for all set successfully? "+result);
                if (result) {
                  main.log_turbowin_system_message(
                      "[EMAIL] success when setting execute permission of " + python_email_exe);
                } else {
                  main.log_turbowin_system_message(
                      "[EMAIL] failed when setting execute permission of " + python_email_exe);
                  python_email_module_status = 1;
                }
              } else {
                // System.out.println("Sorry...File doesn't exist.");
                main.log_turbowin_system_message(
                    "[EMAIL] failed (file not found) when setting execute permission of "
                        + python_email_exe);
                python_email_module_status = 1;
              }
              break;
          ...
          main.log_turbowin_system_message(
              "[EMAIL] error when copying " + resource + " from jar to: " + python_email_dir);
          python_email_module_status = 1;
        ...
        main.log_turbowin_system_message(
            "[EMAIL] error (resource file does not exist) when copying "
                + resource
                + " from jar to: "
                + python_email_dir);
        python_email_module_status = 1;
  ...
  public int send_python_email(
    ...
    if (exeFile.exists() == false) {
      // NB this is an extra check. In theory if the python email exe is not present it should be
      // already copied
    ...
    } else // exitStatus != 100;
    {
      ...
      try {
        ...
        try {
          ...
          //
          // NB see above waitFor(): sometimes BLOCKING !!!! -> use waitFor(long timeout, TimeUnit
          // unit) !!!
          ...
          if (boolean_exit_status
              == true) // it will return a boolean value of true if the subprocess has exited before
          // max waiting time elapsed
          {
            // exitStatus = 0;
            // if the subprocess has been successfully terminated then it will result in an exit
          ...
          // below to be 100% sure there will never be blocking of the main process (=calling
          // thread)
          process.destroy();
  ...
  public final String python_email_exe_return_status_to_text(int exit_status) {
    ...
    switch (exit_status) {
      case 0:
        exit_status_text = "mail sent successfully";
        break;

      // in python module defined exit_status codes
      case 1:
        exit_status_text = "invalid number of arguments";
        break;
      case 2:
        exit_status_text = "invalid smtp_mode";
        break;
      case 3:
        exit_status_text =
      ...
      case 5:
        ...
        break;
      case 6:
        exit_status_text = "The SMTP server refused to accept the message data";
        break;
      case 7:
        exit_status_text = "Error occurred during establishment of a connection with the server";
        break;
      case 8:
        exit_status_text = "The server refused our HELO message";
        break;
      case 9:
        exit_status_text = "The command or option attempted is not supported by the server";
        break;
      case 10:
        exit_status_text =
            "SMTP authentication went wrong. Most probably the server did not accept the username/password combination provided";
        break;
      case 11:
        exit_status_text =
      ...
      case 13:
        exit_status_text =
            "not specified error; internet connection ok?; server reachable?; your email adress valid?; mail port open?";
        break;
      case 14:
        exit_status_text = "Unable to add attachment format 101 obs (HPK_format_101.txt)";
        break;
      case 15:
        exit_status_text =
            "Error, raised to signal an error from the underlying SSL implementation.";
        break;
      case 16:
        exit_status_text =
            "Error, raised when trying to read or write and the SSL connection has been closed cleanly. Note that this does not mean that the underlying transport (read TCP) has been closed.";
        break;
      case 17:
        exit_status_text =
            "Error, raised by a non-blocking SSL socket when trying to read or write data, but more data needs to be received on the underlying TCP transport before the request can be fulfilled.";
        break;
      case 18:
        exit_status_text =
            "Error, raised by a non-blocking SSL socket when trying to read or write data, but more data needs to be received on the underlying TCP transport before the request can be fulfilled.";
      ...
      case 19:
        exit_status_text =
            "Error, raised by a non-blocking SSL socket when trying to read or write data, but more data needs to be sent on the underlying TCP transport before the request can be fulfilled.";
        break;
      case 20:
        exit_status_text =
            "Error, raised when a system error was encountered while trying to fulfill an operation on a SSL socket. Unfortunately, there is no easy way to inspect the original errno number.";
        break;
      case 21:
        exit_status_text =
            "Error, raised when the SSL connection has been terminated abruptly. Generally, you should not try to reuse the underlying transport when this error is encountered.";
        break;
      case 22:
        exit_status_text = "Error, raised when certificate validation has failed.";
        break;

      // in this Java file defined exit_status codes
      case 100:
        exit_status_text = "Email exe not found";
        break;
      case 101:
        exit_status_text = "InterruptedException in function send_python_email";
        break;
      case 102:
        exit_status_text = "IOException in function send_python_email";
        break;
      case 103:
        exit_status_text =
            "unknown mail sent result because the mail process was terminated (took too long to complete)";
        break;

      // NB reserved exit_status codes (see:
      ...
      default:
        exit_status_text = "unknown exit status (see also log_python_email.txt)";
        break;
    }
  ...
  // private final String PYTHON_EMAIL_EXE_LIN                    = "email_tbw_43";
  private final String PYTHON_EMAIL_EXE_LIN_DEB = "email_tbw_43_deb";
  private final String PYTHON_EMAIL_EXE_LIN_RED = "email_tbw_43_red";
  private final String PYTHON_EMAIL_EXE_WIN = "email_tbw_43.exe";
