...
public final class mystationdata extends javax.swing.JFrame {
  ...
  private void initComponents() {
        ...
        jPanel2Layout
            ...
            .addGroup(
                jPanel2Layout
                    ...
                    .addContainerGap()
                    .addComponent(
                        jScrollPane2, javax.swing.GroupLayout.DEFAULT_SIZE, 413, Short.MAX_VALUE)
                    .addContainerGap()));
    ...
    jLabel14.setText("average height anemometer above WL (metres, [0-100], rounded)***");
    ...
    jLabel3.setText("instrument correction (ic)** (hPa, e.g. -0.1)");
    ...
    jRadioButton7.setText("sling psychro.");
    ...
    jRadioButton14.setText("\"through hull\"sens.");

    buttonGroup2.add(jRadioButton15);
    jRadioButton15.setText("radiation thermo.");

    buttonGroup2.add(jRadioButton16);
    jRadioButton16.setText("bait tanks thermo.");
    ...
    jLabel20.setText("* by definition if AWS with wind sensor is connected    ** Summer Load Line");
    ...
    jLabel17.setText("* consult your PMO or National Meteorological Service");

    jLabel22.setText("* always \"no\" if a barometer or an AWS is connected");

    jLabel9.setText(
        "*** only if AWS with wind sensor is connected     **** negative if SLL below WL");

    jLabel4.setText(
        "** average over the whole calibration range, this is NOT the height correction (always 0.0 if AWS connected)");
  ...
  private void lees_iso_landen_codes() {
    ...
    new SwingWorker<Void, Void>() {
      ...
      @Override
      protected void done() {
        // NB open the GUI form, select the JList component, go to Properties -> Code and set "Type
        // Parameters" property to <String>.

        // jList2.setModel(new javax.swing.AbstractListModel()
        ListModel<String> iso_landen_model =
  ...
  private void OK_button_actionPerformed(
    ...
    if (jRadioButton1.isSelected() == true) main.wind_source = main.ESTIMATED_TRUE;
    else if (jRadioButton2.isSelected() == true) main.wind_source = main.MEASURED_OFF_BOW;
    else if (jRadioButton17.isSelected() == true) main.wind_source = main.MEASURED_TRUE;

    // wind units observed/measured
    if (jRadioButton9.isSelected() == true) main.wind_units = main.KNOTS;
    else if (jRadioButton10.isSelected() == true) main.wind_units = main.M_S;
    ...
    // average height anemometer above WL
    main.height_anemometer = jTextField9.getText().trim();

    if (jRadioButton3.isSelected() == true) main.sst_exposure = main.INTAKE;
    else if (jRadioButton4.isSelected() == true) main.sst_exposure = main.BUCKET;
    else if (jRadioButton5.isSelected() == true) main.sst_exposure = main.HULL_CONTACT_SENSOR;
    else if (jRadioButton6.isSelected() == true) main.sst_exposure = main.OTHER;
    else if (jRadioButton13.isSelected() == true) main.sst_exposure = main.TRAILING_THERMISTOR;
    else if (jRadioButton14.isSelected() == true) main.sst_exposure = main.THROUGH_HULL_SENSOR;
    else if (jRadioButton15.isSelected() == true) main.sst_exposure = main.RADIATION_THERMOMETER;
    else if (jRadioButton16.isSelected() == true) main.sst_exposure = main.BAIT_TANKS_THERMOMETER;

    if (jRadioButton7.isSelected() == true) main.air_temp_exposure = main.SLING_PSYCHROMETER;
    else if (jRadioButton8.isSelected() == true) main.air_temp_exposure = main.MARINE_SCREEN;

    // turned off, always UTC taken, if necessary after conversion from other time zone to UTC (see
    // function check_and_set_datetime() in main.java
    //
    ...
    else main.recruiting_country = "";

    // turned off, now fixed on SEA_AND_SWELL_ESTIMATED
    // if (jList1.getSelectedValue() != null)                                 // nothing selected,
    // returns null
    //   main.method_waves             = (String)jList1.getSelectedValue();
    // else
    //   main.method_waves             = "";
    main.method_waves = main.SEA_AND_SWELL_ESTIMATED;

    /* checks */
    if (main.ship_name.trim().equals("") == true || main.ship_name.trim().length() < 2) {
      info = "Ship name: unknown/not valid";
    }
    // from version 4.2: IMO not mandatory anymore
    ...
    else if (main.station_ID.trim().equals("") == true || main.station_ID.trim().length() < 2) {
      info = "station ID: unknown/not valid";
    }

    // else if (main.time_zone_computer.trim().equals("") == true ||
    // main.time_zone_computer.trim().length() < 2)
    // {
    //   info = "Time zone computer: unknown/not valid";
    // }
    else if (main.recruiting_country.trim().equals("") == true
        || main.recruiting_country.trim().length() < 2) {
    ...
    } else if (main.max_height_deck_cargo.trim().equals("") == true
        || main.max_height_deck_cargo.trim().length() < 1) {
    ...
    // only for AWS ??
    // else if (main.wind_units_dashboard.trim().equals("") == true ||
    // main.wind_units_dashboard.trim().length() < 2)
    // {
    //   info = "Wind speed units dashboard unknown/not valid";
    // }

    else if (main.pressure_reading_msl_yes_no.trim().equals("") == true
        || main.pressure_reading_msl_yes_no.trim().length() < 2) {
      info = "Air pressure reading indication: unknown/not valid";
    } else if (main.barometer_above_sll.trim().equals("") == true
    ...
    } else if (main.air_temp_exposure.trim().equals("") == true
        || main.air_temp_exposure.trim().length() < 2) {
    ...
    //
    // NOTE BUT.. observers can also insert call sign in this field so maximum length = 7
    //
    if (doorgaan == true) {
      if (main.station_ID.trim().equals("") == false) {
    ...
    if (doorgaan == true) {
      ...
      try {
        ...
        if (float_barometer_above_sll < 0.0 || float_barometer_above_sll > 100.0) {
          info = "Height of the barometer above SLL must be in range 0.0 - 100.0 metres";
          JOptionPane.showMessageDialog(
              null, info, main.APPLICATION_NAME + " error", JOptionPane.WARNING_MESSAGE);
          doorgaan = false;
        }
    ...
    if (doorgaan == true) {
      ...
      try {
        ...
        if (double_barometer_instrument_correction < -4.0
          ...
          JOptionPane.showMessageDialog(
              null,
              "barometer instrument correction must be in range -4.0 - 4.0",
    ...
    if (doorgaan == true) {
      ...
      if (main.recruiting_country.indexOf("UNITED KINGDOM") != -1) {
        // JOptionPane.showMessageDialog(null, main.recruiting_country, main.APPLICATION_NAME + "
        // test uk ok", JOptionPane.WARNING_MESSAGE);

        // see also myemailsettings.java
        // NB omdat na invullen van dit scherm niet direct emailsettings invul scherm geopend wordt
        //    hier direct de obs_email_subject zetten en vastleggen in configuratie file voor UK
        // recruited ships
  ...
  private void import_button_actionPerformed(
    ...
    String info =
        "If this is not the first time use of this program then you can import previous (automatically) saved station data\n";
  ...
  private void initStationComponents() {
    ...
    if (main.wind_source.trim().indexOf("estimated") != -1) jRadioButton1.setSelected(true);
        ...
        && (main.wind_source.trim().indexOf("true") != -1)) jRadioButton17.setSelected(true);

    jTextField7.setText(main.max_height_deck_cargo.trim());
    jTextField8.setText(main.diff_sll_wl.trim());

    if (main.wind_units.trim().indexOf(main.KNOTS) != -1) jRadioButton9.setSelected(true);
    else if (main.wind_units.trim().indexOf(main.M_S) != -1) jRadioButton10.setSelected(true);
    else // if nothing selected before set selection to "KNOTS"
    jRadioButton9.setSelected(true);
    // JOptionPane.showMessageDialog(null, main.wind_units, main.APPLICATION_NAME + " TEST",
    // JOptionPane.WARNING_MESSAGE);
    ...
    // height anemometer above the WL
    //
    jTextField9.setText(main.height_anemometer.trim());
    ...
    if (main.barometer_instrument_correction.compareTo("") != 0) {
      ...
      // as agreed PB July 2024, now put 0.0 as default here
    ...
    // sst exposure
    //
    if (main.sst_exposure.trim().equals(main.INTAKE)) jRadioButton3.setSelected(true);
    else if (main.sst_exposure.trim().equals(main.BUCKET)) jRadioButton4.setSelected(true);
    else if (main.sst_exposure.trim().equals(main.HULL_CONTACT_SENSOR))
      jRadioButton5.setSelected(true);
    else if (main.sst_exposure.trim().equals(main.OTHER)) jRadioButton6.setSelected(true);
  ...
  private String[] iso_landen_regels =
      new String[MAX_AANTAL_ISO_LANDEN_REGELS]; // default values: null
