...
public class ship {
  ...
  public ship() {
    ...
    color_deck2_ferry =
        new Color(123, 163, 201); // bridge deck      // between steel blue and light steel blue
    ...
    color_pool_1 =
        new Color(
            100, 149,
            237); // starboard and aft swimming pool edge; color between inner swimming pool and
    // outer pool; for depth effect
    color_pool_2 = new Color(0, 0, 205); // inner swimming pool (pool bottom)
    color_pool_3 =
        new Color(
            0, 255, 255,
            100); // outer swimming pool (covering pool bottom and edges (semi transparent)
    ...
    color_deck_steel_blue =
        new Color(176, 196, 222, 255); // light steel blue (NB alpha 255 = 100% opaque)
  ...
  public void draw_general_cargo_ship(
    ...
    //
    // eg see
    // http://what-when-how.com/introduction-to-computer-graphics-using-java-2d-and-3d/basic-principles-of-two-dimensional-graphics-introduction-to-computer-graphics-using-java-2d-and-3d-part-2/
    //
    ...
    //
    if (deck_color != null) {
      color_deck_general_cargo_ship =
          deck_color; // NB if var deck_color not known -> default will be used
    }
    ...
    g2d.draw(
        new Line2D.Double(
            0, wind_rose_diameter / 2 + ship_breadth, 0, -wind_rose_diameter / 2 - ship_breadth));

    //////////// bow ////////
    //
    x = 0; // control point quadTo; relative to origin (centre of wind rose)
    double bow_height_virtual =
        ship_breadth
            * 2; // NB factor 2.0 is indication of the bow curve; quadTo; quadTo; vertical distance
    // from control point to start end end point
    y =
        -(wind_rose_diameter / 2.0)
            + (bow_height_virtual
                * 0.1); // NB factor 0.1 depends on factor used in bow_height_virtual
    ...
    x2 = ship_breadth / 2.0;
    y2 = y1;
    double ship_length_main =
        Math.abs(y1)
            * 2.3; // so the length of the main mid section; NB factor 2.3 depends on the factor
    // used at bow_height_virtual and y
    ...
    {
      if (y_hatch + (hatch_length + hatch_intermediate / 2)
          < (ship_length_main / 2 - bridge_height)) {
        if (night_mode) {
          g2d.setPaint(color_medium_gray);
        } else {
          // if ((color_deck_general_cargo_ship == DASHBOARD_view_APR_radar.DECK_COLOR_GREEN) ||
          // (color_deck_general_cargo_ship == DASHBOARD_view_APR_radar.DECK_COLOR_LIGHT_GREEN))
          // if ((color_deck_general_cargo_ship.getRGB() ==
          // DASHBOARD_view_APR_radar.DECK_COLOR_GREEN.getRGB()) ||
          // (color_deck_general_cargo_ship.getRGB() ==
          // DASHBOARD_view_APR_radar.DECK_COLOR_LIGHT_GREEN.getRGB()))
          if ((color_deck_general_cargo_ship.getRGB() == DECK_COLOR_GREEN.getRGB())
              || (color_deck_general_cargo_ship.getRGB() == DECK_COLOR_LIGHT_GREEN.getRGB())) {
        ...
        for (int k = 0; k < 100; k++) {
          if (y_sub_hatch < (y_hatch + hatch_length)) {
            g2d.draw(
                new Line2D.Double(x_hatch, y_sub_hatch, (x_hatch + hatch_breath), y_sub_hatch));
    ...
    for (int p = 0; p < 3; p++) {
      ...
      Shape shape_crane_pillar =
          new Ellipse2D.Double(
              x_pillar_crane, y_pillar_crane, width_crane_pillar, height_crane_pillar);
    ...
    double y3_first_arm =
        y1_second_arm - width_crane_pillar; // NB y1_first_arm to y3_first_arm - 4 = arm length
    double y3_second_arm =
        y1_third_arm - width_crane_pillar; // NB y1_second_arm to y3_second_arm - 4 = arm length
    double y3_third_arm =
        y4 - width_crane_pillar; // NB y1_third_arm to y4 (start bridge) - 4 = arm length
  ...
  public void draw_tall_ship(Graphics2D g2d, double wind_rose_diameter, boolean night_mode) {
    ...
    try {
      num_masts =
          Integer.parseInt(
              main.ship_type_dashboard.substring(main.ship_type_dashboard.length() - 1));
    ...
    g2d.draw(
        new Line2D.Double(
            0, wind_rose_diameter / 2 + ship_beam, 0, -wind_rose_diameter / 2 - ship_beam));
    ...
    double bow_height_virtual =
        ship_beam
            * 2.2; // NB factor 'x.x' (e.g. 2.2) is indication of the bow curve; quadTo; quadTo;
    // vertical distance from control point to start end end point
    y =
        -(wind_rose_diameter / 2.0)
            + (bow_height_virtual
                * 0.4); // NB factor 0.4 depends on factor used in bow_height_virtual
    ...
    double ship_length =
        Math.abs(y1)
            * 2.3; // so the length of the main mid section; nb factor 'x.x' (e.g. 2.3) depends on
    // the factor used at bow_height_virtual and y
    ...
    double stern_height_virtual =
        ship_beam
            * 1.6; // NB factor 'x.x' is indication of the stern curve; quadTo; quadTo; vertical
    // distance from control point to start end end point
    ys =
        (wind_rose_diameter / 2.0)
            - (stern_height_virtual
                * 1.0); // NB factor 1.0 depends on factor used in bow_height_virtual
    ...
    mast_thickness = 4;
    dist_between_masts =
        ship_length / num_masts + 2; // NB ship_length = the length of the main mid section
    ...
    {
      double masts_offset_1 =
          dist_between_masts * 0.3; // relative to y1 + next masts for ship with 3 masts
      double masts_offset_2 =
          dist_between_masts * 0.2; // relative to y1 + next masts for ship with 4 or 5 masts
      for (int i = 0; i < num_masts; i++) {
        ...
        double x_mast =
            x1
                + (ship_beam / 2)
                - (diameter_mast
                    / 2); // x_mast: The X coordinate of the upper-left corner of the framing
        // rectangle of this Ellipse2D.
        // y_mast: The Y coordinate of the upper-left corner of the framing rectangle of this
        // Ellipse2D.'
        if (category.equals(BARQUE)
            && (i == num_masts - 1)) // aftmost mast -mizzen- fore-and-aft rigged
        {
          if (num_masts == 3) {
          ...
          double x_sail =
              x1_mast
                  - (mast_thickness
                      * 0.5); // the x coordinate of the upper-left corner of the arc to be drawn.
          double y_sail =
              y1_mast
                  + (0 * sail_width); // the y coordinate of the upper-left corner of the arc to be
          // drawn.
          ...
          g2d.draw(
              new Arc2D.Double(x_sail, y_sail, width, height, startAngle, arcAngle, Arc2D.OPEN));

          /////// yard aftmost mast (mizzen) barque
          double yard_length =
              ((x2 + (ship_beam / 2)) - (x1 - (ship_beam / 2)))
                  * 0.75; // so length yard mizzen = 75% of the length of the main mast yard
          g2d.setColor(color_masts_tall_ship);
        ...
        else // not the afmost mast -or- [the aftmost mast but not fore-and-aft rigged]
        {
          ...
          for (int k = 0; k < num_sails; k++) {
            double x_sail =
                x1_mast
                    + (k
                        * sail_width); // the x coordinate of the upper-left corner of the arc to be
            // drawn.
            double y_sail =
                y_mast
                    - mast_thickness; // the y coordinate of the upper-left corner of the arc to be
            // drawn.
      ...
      if (num_masts > 3) {
        ...
        g2d.fill3DRect(
            (int) x_acc_1, (int) y_acc_1, (int) width_acc_1, (int) height_acc_1, raised_acc_1);
      }
      ...
      g2d.fill3DRect(
          (int) x_acc_2, (int) y_acc_2, (int) width_acc_2, (int) height_acc_2, raised_acc_2);
  ...
  public void draw_yacht(Graphics2D g2d, double wind_rose_diameter, boolean night_mode) {
    ...
    double bow_height_virtual =
        ship_beam
            * 3.4; // NB factor 'x.x' (e.g. 2.2) is indication of the bow curve; quadTo; quadTo;
    // vertical distance from control point to start end end point
    y =
        -(wind_rose_diameter / 2.2)
            + (bow_height_virtual
                * 0.1); // NB factor 0.4 depends on factor used in bow_height_virtual
    ...
    double ship_length =
        Math.abs(y1)
            * 4.4; // so the length of the main mid section; nb factor 'x.x' (e.g. 2.3) depends on
    // the factor used at bow_height_virtual and y
    ...
    double stern_height_virtual =
        ship_beam
            * 0.1; // NB factor 'x.x' is indication of the stern curve; quadTo; quadTo; vertical
    // distance from control point to start end end point
    ys =
        (wind_rose_diameter / 4.0)
            - (stern_height_virtual
                * 1.0); // NB factor 1.0 depends on factor used in bow_height_virtual
    ...
    int delta =
        6; // horizontal distance between main deck -> first deck -> second deck -> third deck ->
    // fourth deck
    ...
    if (deck_3_width > (delta * 3)) {
      ...
      double y1_pool =
          y1 + (delta * 5) - (delta * 2) + deck_3_height + delta; // aft line third deck + delta
      ...
      g2d.fillOval(
          ...
          (int) y1_pool,
          (int) pool_width,
          (int) pool_height); // This fills a circle or an oval that fits within the rectangle
      // specified by the X, Y, width and height arguments.
      // The oval is drawn inside a rectangle whose upper left hand corner is at (x_heli, y_heli),
      // and whose width and height are as specified (d).
    ...
    if (deck_4_width > (delta * 4)) {
      ...
      if ((dome_diameter * 4) < deck_4_width) {
        ...
        Shape shape_dome_3 =
            new Ellipse2D.Double(x3_dome, y3_dome, dome_small_diameter, dome_small_diameter);
        Shape shape_dome_4 =
            new Ellipse2D.Double(x4_dome, y4_dome, dome_small_diameter, dome_small_diameter);
    ...
    double width_heli =
        (Math.abs(x1 - x2) - (2 * heli_circle_thickness) - (2 * heli_circle_offset)) / 2;
    ...
    g2d.drawOval(
        x_heli, y_heli, d,
        d); // This draws a circle or an oval that fits within the rectangle specified by the X, Y,
    // width and height arguments.
    // The oval is drawn inside a rectangle whose upper left hand corner is at (x_heli, y_heli), and
    // whose width and height are as specified (d).
  ...
  public void draw_general_cargo_ship_II(
    ...
    //
    if (deck_color != null) {
      color_deck_general_cargo_ship_II =
          deck_color; // NB if var deck_color not known -> default will be used
    }
    ...
    //
    x = 0; // control point quadTo; relative to origin (centre of wind rose)
    // y = -(wind_rose_diameter / 2.0);                                      // control point
    // quadTo; relative to origin (centre of wind rose)
    double bow_height_virtual =
        ship_breadth
            * 2; // NB factor 2.0 is indication of the bow curve; quadTo; quadTo; vertical distance
    // from control point to start end end point
    ...
    for (int i = 0; i < 50; i++) // max 50 hatches
    {
      if (y_hatch + (hatch_length + hatch_intermediate / 2) < dist_origin_bridge) {
        if (night_mode) {
          ...
          if ((color_deck_general_cargo_ship_II.getRGB() == DECK_COLOR_GREEN.getRGB())
              || (color_deck_general_cargo_ship_II.getRGB() == DECK_COLOR_LIGHT_GREEN.getRGB())) {
  ...
  public void draw_general_cargo_ship_III(
    ...
    //
    if (deck_color != null) {
      color_deck_general_cargo_ship_III =
          deck_color; // NB if var deck_color not known -> default will be used
    }
    ...
    for (int p = 0; p < 4; p++) {
      ...
      g2d.draw(
          new Line2D.Double(
              x_hatch,
              y_hatch + (hatch_length / 2),
              x_hatch + hatch_width,
              y_hatch + (hatch_length / 2))); // hatch sections deviders
    } // for (int p = 0; p < 4; p++)
    ...
    for (int p = 0; p < 4; p++) {
      ...
      if (p == 0) // first crane
      {
        ...
        // if the (ship) figure is very small (thickness crane arms), NB only necessary to determine
        // at the first crane
        if (x4_arm < x3_arm) {
          g2d.setStroke(new BasicStroke(2.0f));
        } else // 'normal' figure
    ...
    for (int p = 0; p < 4; p++) {
      if (p == 0) {
        Shape shape_crane_pillar =
            new Ellipse2D.Double(
                x_pillar_crane, y_pillar_crane_1, width_crane_pillar, width_crane_pillar);
        ...
        Shape shape_crane_pillar =
            new Ellipse2D.Double(
                x_pillar_crane, y_pillar_crane_2, width_crane_pillar, width_crane_pillar);
        ...
        Shape shape_crane_pillar =
            new Ellipse2D.Double(
                x_pillar_crane, y_pillar_crane_3, width_crane_pillar, width_crane_pillar);
        ...
        Shape shape_crane_pillar =
            new Ellipse2D.Double(
                x_pillar_crane, y_pillar_crane_4, width_crane_pillar, width_crane_pillar);
  ...
  public void draw_bulk_carrier_II(
    ...
    double bow_height_virtual =
        ship_beam
            * 1.2; // NB factor 'x.x' (e.g. 1.2) is indication of the bow curve; quadTo; quadTo;
    // vertical distance from control point to start end end point (the smaller the
    // factor the blunter the bow)
    y =
        -(wind_rose_diameter / 2.0)
            + (bow_height_virtual
                * 0.3); // NB factor 0.5 depends on factor used in bow_height_virtual (the lower the
    // factor the foreship -heading- is more positioned to the edge of the
    // circle)
    ...
    double ship_length_main =
        Math.abs(y1)
            * 2.05; // so the length of the main mid section; NB factor 2.3 depends on/relates to
    // the factor used at bow_height_virtual and y
    ...
    for (int p = 0; p < 5; p++) {
      ...
      // g2d.fill3DRect((int)x_hatch, (int)y_hatch, (int)hatch_width, (int)hatch_length, true);
      //                                 // raised hatches
      fill3DRectImproved(
          g2d,
          (int) x_hatch,
  ...
  public void draw_sailing_yacht(Graphics2D g2d, double wind_rose_diameter, boolean night_mode) {
    ...
    double bow_height_virtual =
        ship_beam
            * 5.0; // NB factor 'x.x' (e.g. 2.2) is indication of the bow curve; quadTo; quadTo;
    // vertical distance from control point to start end end point
    y =
        -(wind_rose_diameter / 1.9)
            + (bow_height_virtual
                * 0.1); // 1.9 -> greater: more centre circle bow punt    // NB factor 0.1 depends
    // on/related to factor used in bow_height_virtual
    ...
    double ship_length_main =
        Math.abs(y1)
            * 2.0; // 1.0 -> 2.0                             // so the length of the main mid
    // section; nb factor 'x.x' (e.g. 2.3) depends on the factor used at
    // bow_height_virtual and y
    ...
    //
    //
    //  (x1_cockpit,y1_cockpit)-------- curved--------(x2_cockpit,y2_cockpit)          <---- front
    // cockpit
    //                        |                      |
    //                        |                      |
    //                        |                      |
    ...
    GradientPaint canopyGradient =
        new GradientPaint(
            (float) ((x1_cockpit + x2_cockpit) / 2.0),
            (float) y1_cockpit,
            topColor,
            (float) ((x3_cockpit + x4_cockpit) / 2.0),
            (float) y4_cockpit,
            bottomColor);
    ...
    double x_tip =
        (1 - t_tip) * (1 - t_tip) * x1 + 2 * (1 - t_tip) * t_tip * x + t_tip * t_tip * x2;
    double y_tip =
        (1 - t_tip) * (1 - t_tip) * y1
            + 2 * (1 - t_tip) * t_tip * y
            + t_tip * t_tip * y2
            + (ship_beam / 8); // NB forestay is not exactly detached at the tip of the bow

    // mast top connection
    double x_forestay = x_mast + width_mast / 2; // center of mast
    // double y_forestay_top = y_mast + width_mast / 4;                       // attached to the
    // mast (but not exactly in the middle)
    ...
    g2d.fill(
        new Ellipse2D.Double(
            x_tip - attachment_radius,
            y_tip - attachment_radius,
            attachment_radius * 2,
            attachment_radius * 2));
  }
  ...
  public void draw_catamaran(Graphics2D g2d, double wind_rose_diameter, boolean night_mode) {
    ...
    /////////////////////// hulls ////////////////////
    //
    java.util.function.DoubleFunction<GeneralPath> makeHull =
        (double cx) -> {
          double x = cx;
    ...
    bridgedeck.quadTo(
        0.0, bridgedeck_front_y - hull_beam_f * 0.12, bridgedeck_right_x, bridgedeck_front_y);
    ...
    java.util.function.DoubleFunction<GeneralPath> makeCockpit =
        (double cx) -> {
    ...
    GradientPaint cockpitGradientLeft =
        new GradientPaint(
            (float) cx_left,
            (float) cp_front_y,
            topColor,
            ...
            (float) cp_back_y,
            bottomColor);
    GradientPaint cockpitGradientRight =
        new GradientPaint(
            (float) cx_right,
            (float) cp_front_y,
            topColor,
    ...
    g2d.setPaint(cockpitGradientLeft);
    g2d.fill(cockpitLeft);
    g2d.setPaint(cockpitGradientRight);
    g2d.fill(cockpitRight);
    ...
    java.util.function.BiConsumer<Double, Double> drawHelm =
        (Double x1_helm, Double x2_helm) -> {
          double x_mid_helm = (x1_helm + x2_helm) / 2.0;
  ...
  public void draw_heavy_lift_I(
    ...
    double bow_height_virtual =
        ship_beam
            * 1.5; // NB factor 'x.x' (e.g. 2.2) is indication of the bow curve; quadTo; quadTo;
    // vertical distance from control point to start end end point
    y =
        -(wind_rose_diameter / 2.0)
            + (bow_height_virtual
    ...
    double ship_length_main =
        Math.abs(y1)
            * 2.1; // so the length of the main mid section; NB factor 2.3 depends on the factor
    // used at bow_height_virtual and y
    ...
    double yPoints_bridge[] = {
      ...
      y11_bridge,
      y12_bridge
    }; // -values for y goes/points to the bow
  ...
  */
  ...
  public ship(
      Color color_white,
      Color color_black,
      Color color_deck_passenger_ship,
      Color color_pool_1,
      Color color_pool_2,
      Color color_pool_3,
      Color color_pool_4,
      Color color_acc_passenger_ship,
      Color color_life_boat,
      Color color_funnel_passenger_ship,
      Color color_funnel_ferry,
      Color color_bridge_oil_tanker,
      Color color_bridge_container_ship,
      Color color_bridge_ro_ro_ship,
      Color color_bulk_carrier,
      Color color_lng_tanks,
      Color color_deck_lng_tanker,
      Color color_deck_oil_tanker,
      Color color_pipes_oil_tanker,
      Color color_deck_steel_blue,
      Color color_deck_reefer_ship,
      Color color_deck_general_cargo_classic,
      Color color_deck_research_vessel,
      Color color_crane_research_vessel,
      Color color_hoist_research_vessel,
      Color color_cradle_research_vessel,
      Color color_bridge_research_vessel,
      Color color_night_crane_research_vessel,
      Color color_night_hoist_research_vessel,
      Color color_night_cradle_research_vessel,
      Color color_deck_general_cargo_ship,
      Color color_deck_container_ship_II,
      Color color_deck1_ferry,
      Color color_deck2_ferry,
      Color color_deck3_ferry,
      Color color_deck_ro_ro,
      Color color_lanes_ro_ro,
      Color color_hatches_general_cargo_ship,
      Color color_medium_gray,
      Color color_container_1,
      Color color_container_2,
      Color color_container_3,
      Color color_container_4,
      Color color_container_5,
      Color color_container_6,
      Color color_container_7,
      Color color_container_8,
      Color color_container_9,
      Color color_container_10,
      Color color_derricks,
      Color color_deck_general_cargo_ship_II,
      Color color_hatches_general_cargo_ship_II,
      Color color_deck_lng_tanker_II,
      Color color_bridge_lng_tanker_II,
      Color color_tank_deck_fruit_juice_tanker,
      Color color_bridge_fruit_juice,
      Color color_tanks_fruit_juice,
      Color color_aft_deck_fruit_juice_tanker,
      java.awt.Color color_deck_tall_ship,
      java.awt.Color color_masts_tall_ship,
      java.awt.Color color_deck_yacht,
      java.awt.Color color_chemical_tanker,
      Color color_deck_general_cargo_ship_III,
      Color color_deck_heavy_lift,
      Color color_cranes,
      Color color_deck_ro_ro_2) {
    this.color_white = color_white;
    this.color_black = color_black;
    this.color_deck_passenger_ship = color_deck_passenger_ship;
    this.color_pool_1 = color_pool_1;
  ...
  public void draw_neutral_ship(Graphics2D g2d, double wind_rose_diameter, boolean night_mode) {
    ...
    y =
        -(wind_rose_diameter
            / 2.0); // control point quadTo; relative to origin (centre of wind rose)
    double bow_height_virtual =
        ship_breadth * 2.5; // quadTo; vertical distance from control point to start end end point
  ...
  public void draw_research_vessel(Graphics2D g2d, double wind_rose_diameter, boolean night_mode) {
    ...
    double y_control =
        -(wind_rose_diameter
            / 2.0); // control y point quadTo; relative to origin (centre of wind rose)
    bow_height_virtual =
        ship_breadth
            * 2.5; // for quadTo; vertical distance from control point to start end end point
    ...
    QuadCurve2D quad =
        new QuadCurve2D.Double(x1_curve, y1_curve, ctrx_curve, ctry_curve, x2_curve, y2_curve);
    g2d.setStroke(new BasicStroke(2.0f));
    ...
    double y_pillar_crane =
        (y4 - y3_bridge)
            / 2; // crane position in the middle of bridge and where the indention/curve of the aft
    // ship starts
    ...
    double yPoints_container[] = {
      y1_container, y2_container, y3_container, y4_container
    }; // -values for y goes/points to the bow

    GeneralPath polygon_container =
        new GeneralPath(GeneralPath.WIND_EVEN_ODD, xPoints_container.length);
    ...
    double y1_hoist =
        y6 - (ship_length / 17); // NB ship_length = the length of the main mid section
    ...
    // polygon_hoist.closePath();                                            // NB do not close the
    // polygon!
  ...
  public void draw_container_ship_II(
    ...
    //
    x = 0; // control point quadTo; relative to origin (centre of wind rose)
    // double bow_height_virtual = ship_breadth * 1.1;                    // NB factor 1.1 is
    // indication of the bow curve; quadTo; vertical distance from control point to start end end
    // point
    // y = -(wind_rose_diameter / 2.0) + (bow_height_virtual * 0.6);      // NB fator 0.6 depends on
    // factor used in bow_height_virtual
    double bow_height_virtual =
        ship_breadth
            * 2.0; // NB factor 1.1 is indication of the bow curve; quadTo; vertical distance from
    // control point to start end end point
    y =
        -(wind_rose_diameter / 2.0)
            + (bow_height_virtual
                * 0.1); // NB fator 0.6 depends on factor used in bow_height_virtual
    ...
    double ship_length =
        Math.abs(y1)
            * 1.9; // so the length of the main mid section; nb factor 1.65 depends on the factor
    // used at bow_height_virtual and y
    ...
    double
        y_container_offset; // painting on aft ship start with x,y as top left and not bottom left
    ...
    for (int i = 0; i < max_number_containers_rows; i++) {
      ...
      if (doorgaan) {
        ...
        if (i == 0) // first row fore ship (20ft row on the bow)
        {
          ...
          end_in_row =
              number_containers_in_row - 1; // because less container space available on the bow
        } else {
          ...
          } else // day mode
          {
            if (deck_color != null) {
              ...
              } else // no deck color defined or deckcolor = various
              {
                ...
                switch (random) {
                  case 1:
                    g2d.setPaint(color_container_1);
                    break;
                  case 2:
                    g2d.setPaint(color_container_2);
                    break;
                  case 3:
                    g2d.setPaint(color_container_3);
                    break;
                  case 4:
                    g2d.setPaint(color_container_4);
                    break;
                  case 5:
                    g2d.setPaint(color_container_5);
                    break;
                  case 6:
                    g2d.setPaint(color_container_6);
                    break;
                  case 7:
                    g2d.setPaint(color_container_7);
                    break;
                  case 8:
                    g2d.setPaint(color_container_8);
                    break;
                  case 9:
                    g2d.setPaint(color_container_9);
                    break;
                  case 10:
                    g2d.setPaint(color_container_10);
                    break;
                  default:
                    g2d.setPaint(color_container_1);
                    break;
  ...
  public void draw_general_cargo_classic(
    ...
    double bow_height_virtual =
        ship_breadth
            * 2.1; // NB factor 2.0 is indication of the bow curve; quadTo; quadTo; vertical
    // distance from control point to start end end point
    y =
        -(wind_rose_diameter / 2.0)
            + (bow_height_virtual
    ...
    double ship_length =
        Math.abs(y1)
            * 2.1; // so the length of the main mid section; nb factor 1.65 depends on the factor
    // used at bow_height_virtual and y
    ...
    double stern_height_virtual =
        ship_breadth
            * 0.5; // NB factor 2.0 is indication of the bow curve; quadTo; quadTo; vertical
    // distance from control point to start end end point
    ys =
        (wind_rose_diameter / 2.0)
            - (stern_height_virtual
                * 1.0); // NB factor 0.1 depends on factor used in bow_height_virtual
    ...
    if (wind_rose_diameter
        > BRIDGE_DRAW_LIMIT) // >= e.g. when 4K screen resolution + TurboWin+ full screen
    {
      bridge_height = 14;
    }
    ...
    for (int i = 0; i < MAX_HATCHES; i++) // max 20 hatches
    {
      if (y_hatch + (hatch_length + hatch_intermediate / 2) < (dist_origin_bridge)) {
        ...
        if (i == 0) {
          ...
          Shape shape_mast_port =
              new Ellipse2D.Double(
                  x1_mast_fore_port, y1_mast_fore_port, mast_diameter_fore, mast_diameter_fore);
          ...
          Shape shape_mast_sb =
              new Ellipse2D.Double(
                  x1_mast_fore_sb, y1_mast_fore_sb, mast_diameter_fore, mast_diameter_fore);
          g2d.fill(shape_mast_sb);

          // derricks hatch I (defined from fore to aft ship = from hatch I masts to island between
          // hatch I and hatch II)
          y1_derricks_hatch_1_mik = y_hatch + hatch_length + hatch_intermediate / 2;

          g2d.setStroke(new BasicStroke(3.0f));
          g2d.draw(
              new Line2D.Double(
                  (x1_mast_fore_port + (mast_diameter_heavy / 2) / 2),
                  (y1_mast_fore_port + mast_diameter_heavy / 2),
                  (x1_mast_fore_port + mast_diameter_heavy / 2),
                  (y1_derricks_hatch_1_mik - 1)));
          g2d.draw(
              new Line2D.Double(
                  (x1_mast_fore_sb + mast_diameter_heavy / 2),
                  (y1_mast_fore_sb + mast_diameter_heavy / 2),
                  (x1_mast_fore_sb + mast_diameter_heavy / 2),
                  (y1_derricks_hatch_1_mik - 1)));
        } //  if (i == 0)

        // mast between hatch II and hatch III (heavy duty mast/derrick) and derricks hatch II and
        // hatch III
        //
        if (i == 2) {
          ...
          Shape shape_mast_sb =
              new Ellipse2D.Double(
                  x1_mast_heavy, y1_mast_heavy, mast_diameter_heavy, mast_diameter_heavy);
          g2d.fill(shape_mast_sb);

          // derricks hatch II (defined from aft to fore ship = from heavy duty mast to island
          // between hatch II and hatch III))
          g2d.setStroke(new BasicStroke(3.0f));
          g2d.draw(
              new Line2D.Double(
                  (x1_mast_heavy),
                  (y1_mast_heavy + mast_diameter_heavy / 2),
                  (x1_mast_fore_port + mast_diameter_heavy / 2),
                  y1_derricks_hatch_1_mik + 1));
          g2d.draw(
              new Line2D.Double(
                  (x1_mast_heavy + mast_diameter_heavy),
                  (y1_mast_heavy + mast_diameter_heavy / 2),
                  (x1_mast_fore_sb + mast_diameter_heavy / 2),
                  y1_derricks_hatch_1_mik + 1));

          // derricks hatch III (defined from aft to fore ship = from ship bridge to heavy duty
          // mast))
          g2d.setStroke(new BasicStroke(3.0f));
          g2d.draw(
              new Line2D.Double(
                  (x1 + 2),
                  (dist_origin_bridge - 2),
                  x1_mast_heavy,
                  (y1_mast_heavy + mast_diameter_heavy / 2)));
          g2d.draw(
              new Line2D.Double(
    ...
    if (y_bridge_aft < y3) {
      ...
      Shape shape_mast_port_aft =
          new Ellipse2D.Double(
              x1_mast_aft_port, y1_mast_aft_port, mast_diameter_aft, mast_diameter_aft);
      ...
      Shape shape_mast_sb_aft =
          new Ellipse2D.Double(
              x1_mast_aft_sb, y1_mast_aft_sb, mast_diameter_aft, mast_diameter_aft);
      g2d.fill(shape_mast_sb_aft);

      // derricks hatch IV (defined from fore to aft ship = from hatch IV masts to island after
      // hatch IV)
      ...
      g2d.draw(
          new Line2D.Double(
              (x1_mast_aft_port + (mast_diameter_aft / 2) / 2),
              (y1_mast_aft_port + mast_diameter_aft / 2),
              (x1_mast_heavy),
              y1_derricks_hatch_4_mik));
      g2d.draw(
          new Line2D.Double(
              (x1_mast_aft_sb + mast_diameter_aft / 2),
              (y1_mast_aft_sb + mast_diameter_aft / 2),
              (x1_mast_heavy + mast_diameter_heavy),
              y1_derricks_hatch_4_mik));
    } // if (y_bridge_aft < y3)
  }
  ...
  public void draw_reefer_ship(Graphics2D g2d, double wind_rose_diameter, boolean night_mode) {
    ...
    for (int i = 0; i < MAX_HATCHES; i++) // max 20 hatches
    {
      if (y_hatch + (hatch_length + hatch_intermediate / 4) < (dist_origin_bridge)) {
        ...
        double crane_arm_length =
            y_hatch + hatch_length + (hatch_intermediate / 2) - (width_crane_pillar * 0.75);
        draw_crane(
            g2d,
            night_mode,
            ship_breadth,
  ...
  public void draw_lng_tanker_II(
    ...
    g2d.draw(
        new Line2D.Double(
            x3_front_tank_port, y3_front_tank_port, x3_front_tank_sb, y3_front_tank_sb));
    ...
    for (int i = 0; i < 6; i++) {
      Shape shape_pipe =
          new Line2D.Double(x_pipe, y_pipe, x_pipe + ship_breadth - (2 * between_pipes), y_pipe);
      g2d.draw(shape_pipe);
    ...
    if (((y4 / 2) + platform_length)
        < (dist_origin_bridge - bridge_height)) // only if there is enough room to draw
    {
  ...
  public void draw_fruit_juice_tanker(
    ...
    ////////////////// fill aft ship (fruit juice tanker different color aft deck compared to main
    // section and bow)
    //
    ...
    g2d.draw(
        new Line2D.Double(
            x4_front_tank_port, y4_front_tank_port, x4_front_tank_sb, y4_front_tank_sb));
    ...
    for (int i = 0; i < 3; i++) {
      ...
      double xPoints_deck_line[] = {
        x1_pipe, x2_pipe, x3_pipe, x4_pipe
      }; // -values for x goes/points to port (relative to origin)
      double yPoints_deck_line[] = {
        y1_pipe, y2_pipe, y3_pipe, y4_pipe
      }; // -values for y goes/points to the bow( relative to origin)
        ...
        < (dist_origin_bridge - bridge_height)) // only if there is enough room to draw
    {
      ...
      for (int i = 0; i < 3; i++) {
        Shape shape_pipe =
            new Line2D.Double(
                x_platform_pipe, y_platform_pipe_aft, x_platform_pipe, y_platform_middle);
        g2d.draw(shape_pipe);
      ...
      for (int i = 0; i < 2; i++) {
        Shape shape_pipe =
            new Line2D.Double(
                x_platform_pipe, y_platform_middle, x_platform_pipe, y_platform_pipe_front);
        g2d.draw(shape_pipe);
        x_platform_pipe -= between_pipes;
      }
    ...
    Shape shape_front_side =
        new Ellipse2D.Double(x_front_circle, y_front_circle, circle_diameter, circle_diameter);
    Shape shape_aft_side =
        new Ellipse2D.Double(x_aft_circle, y_aft_circle, circle_diameter, circle_diameter);
    if (night_mode) {
      g2d.setPaint(color_medium_gray);
    } else {
      g2d.setPaint(color_tanks_fruit_juice);
  ...
  public void draw_crane(
      ...
      boolean night_mode,
      double ship_breadth,
      double x_pillar_crane,
      double y_pillar_crane,
      double crane_arm_length,
      double width_crane_pillar,
      Color crane_color) {
    ///////////////////////// crane pillars ////////////////////////
    //
    //
  ...
  public void draw_ro_ro_ship_I(Graphics2D g2d, double wind_rose_diameter, boolean night_mode) {
    ...
    double bow_height_virtual =
        ship_breadth
            * 1.1; // NB factor 1.1 is indication of the bow curve; quadTo; vertical distance from
    // control point to start end end point
    y =
        -(wind_rose_diameter / 2.0)
            + (bow_height_virtual
                * 0.6); // NB fator 0.6 depends on factor used in bow_height_virtual
    ...
    double ship_length =
        Math.abs(y1)
            * 1.65; // so the length of the main mid section; nb factor 1.65 depends on the factor
    // used at bow_height_virtual and y
    ...
    g2d.draw(
        new Line2D.Double(
            x10_bridge, y10_bridge - between_acc, x9_bridge, y9_bridge - between_acc));
  ...
  public void draw_ro_ro_ship_II(
    ...
    x = 0; // control point quadTo; relative to origin (centre of wind rose)
    double bow_height_virtual =
        ship_beam * 1.5; // NB factor 1.5 is indication of the bow curve; quadTo; quadTo; vertical
    // distance from control point to start end end point
    y =
        -(wind_rose_diameter / 2.0)
            + (bow_height_virtual
                * 0.3); // NB factor 0.3 depends on factor used in bow_height_virtual
    ...
    double ship_length_main =
        Math.abs(y1)
            * 1.75; // so the length of the main mid section; NB factor 2.3 depends on the factor
    // used at bow_height_virtual and y
    ...
    double dist_bow_hatch_1 =
        ship_beam / 4; // space between (back of the) bow to hatch number 1 (first hatch)
    double hatch_3_dist_origin_bridge =
        dist_between_hatches; // space between back of hatch 3 (last hatch) and the bridge structure
    double deck_length_hatches =
        abs(y1)
            + dist_origin_bridge
            - dist_bow_hatch_1
            - hatch_3_dist_origin_bridge
            - (dist_between_hatches * 4); // total effective deck length available for hatches
    ...
    for (int p = 0; p < 4; p++) {
      ...
      } else // side ramp structure
      {
        ...
        // g2d.fill3DRect((int)x1_side_ramp, (int)y1_side_ramp, (int)width, (int)height, true);
        //                               // raised hatches
        fill3DRectImproved(
            g2d,
            (int) x1_side_ramp,
    ...
    // NB this cumbersone construction to get the bow totally white (and not spoiled with a few
    // pixels red)
  ...
  public void draw_ferry(Graphics2D g2d, double wind_rose_diameter, boolean night_mode) {
    ...
    double bow_height_virtual =
        ship_breadth
            * 1.2; // NB factor 1.2 indicates the bow curve; quadTo; vertical distance from control
    // point to start end end point
    y =
        -(wind_rose_diameter / 2.0)
            + (bow_height_virtual
                * 0.5); // NB factor 0.5 depends on factor used in bow_height_virtual
    ...
    double ship_length =
        Math.abs(y1)
            * 1.7; // so the length of the main mid section; nb factor 1.7 depends on the factor
    // used at bow_height_virtual and y
    ...
    double yPoints_top_deck[] = {
      y13_bridge, y14_bridge, y15_bridge, y16_bridge
    }; // -values for y goes/points to the bow

    GeneralPath polygon_top_deck =
        new GeneralPath(GeneralPath.WIND_EVEN_ODD, xPoints_top_deck.length);
    ...
    g2d.fill(
        new RoundRectangle2D.Double(x1_funnel, y1_funnel, width_funnel, length_funnel, 20, 20));
  ...
  public void draw_container_ship(
    ...
    double y =
        -(wind_rose_diameter / 2.0 - ship_breadth); // relative to origin (centre of wwind rose)
    ...
    double bow_height =
        ship_breadth
            * 2; // total height of the ellipse, bur only the upper half will be visible as the bow
    Shape shape_bow = new Ellipse2D.Double(x, y, bow_width, bow_height);

    g2d.setStroke(new BasicStroke(1.0f));
    g2d.draw(
        shape_bow); // point East = 0 degrees (normally you would call East 90 degrees...) -> to the
    // left
    ...
    double y_container;
    boolean doorgaan = true;
    double offset_y_bridge =
        0; // for cheking space between bridge and first container row for and aft the bridge
    double
        y_container_offset; // painting on aft ship start with x,y as top left and not bottom left
    ...
    for (int m = 0; m < 2; m++) // two loops (fore and aft ship)
    {
      ...
      } else // aft ship
      {
        y_container =
            Math.abs(y1)
                + stern; // ship_length / 2;                              // now y_container is
        // positive value (-values for y goes to the bow from the origin
      }
      ...
      for (int i = 0; i < max_number_containers_rows; i++) {
        ...
        if (doorgaan) {
          ...
          {
            y_container_offset =
                0; // because of the starting point (x,y) of the drawing of a rectangle
          } else // aft ship
          {
            y_container_offset =
                container_length; // because of the starting point (x,y) of the drawing of a
            // rectangle
          }
  ...
  public void draw_oil_tanker(
    ...
    //
    if (deck_color != null) {
      color_deck_oil_tanker =
          deck_color; // NB if var deck_color not known -> default (= var color_deck_oil_tanker)
      // will be used
    }
    ...
    double x = -ship_breadth / 2.0; // relative to origin (centre of wind rose)
    double y =
        -(wind_rose_diameter / 2.0 - ship_breadth); // relative to origin (centre of wind rose)
    ...
    double bow_height =
        ship_breadth
            * 1.7; // total height of the ellipse, bur only the upper half will be visible as the
    // bow
    Shape shape_bow = new Ellipse2D.Double(x, y, bow_width, bow_height);
    ...
    for (int i = 0; i < 3; i++) {
      Shape shape_pipe =
          new Line2D.Double(
              x_pipe, y_pipe, x_pipe, y_pipe + ship_length - bridge_height - between_pipes);
      g2d.draw(shape_pipe);
      x_pipe += between_pipes;
    }
    ...
    for (int i = 0; i < 3; i++) {
      Shape shape_pipe =
          new Line2D.Double(
              x_pipe, y_pipe, x_pipe, y_pipe + (ship_length / 2) - bridge_height - between_pipes);
      g2d.draw(shape_pipe);
      x_pipe -= between_pipes;
    }
    ...
    for (int i = 0; i < 2; i++) {
      ...
      double xPoints_deck_line[] = {
        x0_deck_line, x1_deck_line, x2_deck_line, x3_deck_line
      }; // -values for x goes/points to port (relative to origin)
      double yPoints_deck_line[] = {
        y0_deck_line, y1_deck_line, y2_deck_line, y3_deck_line
      }; // -values for y goes/points to the bow( relative to origin)
  ...
  public void draw_lng_tanker(
    ...
    double bow_height =
        ship_breadth
            * 2.0; // total height of the ellipse, bur only the upper half will be visible as the
    // bow
    Shape shape_bow = new Ellipse2D.Double(x, y, bow_width, bow_height);
    ...
    ///////////// LNG tanks //////////
    //
    double tank_intermediate =
        ship_breadth / 10; // distances between 2 tanks and between tank and railing
  ...
  public void draw_bulk_carrier(
    ...
    if (deck_color != null) {
      color_bulk_carrier =
          deck_color; // NB if var deck_color not known -> default (= var color_bulk_carrier) will
      // be used
    }
    if (night_mode) // in night mode always dark-gray
    {
    ...
    double bow_height =
        ship_breadth
            * 1.2; // total height of the ellipse, bur only the upper half will be visible as the
    // bow
    Shape shape_bow = new Ellipse2D.Double(x, y, bow_width, bow_height);
    ...
    for (int i = 0; i < 20; i++) // max 20 hatches
    {
      if (y_hatch + (hatch_length + hatch_intermediate / 2) < (ship_length / 2 - bridge_height)) {
        g2d.setPaint(color_bulk_carrier);
        // g2d.fill3DRect((int) x_hatch, (int) y_hatch, (int) hatch_breath, (int) hatch_length,
        // true);   // raised hatches
        fill3DRectImproved(
            g2d,
            (int) x_hatch,
  ...
  private void draw_ship_bridge(
      ...
      double acc_height,
      double dist_origin_bridge,
      double bridge_indention,
      boolean night_mode) {
    ...
    double xPoints[] = {
      x1_bridge, x2_bridge, x3_bridge, x4_bridge, x5_bridge, x6_bridge, x7_bridge, x8_bridge
    };
    double yPoints[] = {
      y1_bridge, y2_bridge, y3_bridge, y4_bridge, y5_bridge, y6_bridge, y7_bridge, y8_bridge
    }; // -values for y goes/points to the bow
    ...
    if (main.ship_type_dashboard.equals(main.OIL_TANKER)
        || main.ship_type_dashboard.equals(main.LNG_TANKER_II)) {
      if (night_mode) {
        g2d.setColor(color_medium_gray);
      } else {
        g2d.setColor(color_white);
  ...
  private double draw_ship_bridge_classic(
    ...
    double ctrx_curve = 0;
    double ctry_curve =
        dist_origin_bridge - 15; // -15: curve of the bridge, greather value = more curve
    ...
    g2d.setPaint(Color.BLACK);
    g2d.fill(
        new RoundRectangle2D.Double(x1_funnel, y1_funnel, funnel_width, funnel_length, 40, 40));
  ...
  public void draw_passenger_ship(Graphics2D g2d, double wind_rose_diameter, boolean night_mode) {
    ...
    for (int i = 0; i < 3; i++) {
      QuadCurve2D quad =
          new QuadCurve2D.Double(
              x1_curve + i, y1_curve, ctrx_curve, ctry_curve, x2_curve - i, y2_curve);
      g2d.setColor(color_black);
      g2d.draw(quad);
    ...
    */
    ...
    if (dive_platform_width < pool_width - (2 * pool_edge)) {
      ...
      double yPoints[] = {
        y1_dive, y2_dive, y3_dive, y4_dive, y5_dive, y6_dive, y7_dive, y8_dive
      }; // -values for y goes/points to the bow
  ...
  public void draw_chemical_tanker(
    ...
    //
    if (deck_color != null) {
      color_chemical_tanker =
          deck_color; // NB if var deck_color not known -> default (= var color_chemical_tanker)
      // will be used
    ...
    double ship_length_main =
        Math.abs(y1)
            * 2.05; // so the length of the main mid section; nb factor 'x.x' (e.g. 2.3) depends on
    // the factor used at bow_height_virtual and y
    ...
    for (int i = 0; i < number_frames; i++) {
      Shape shape_frame =
          new Line2D.Double(x1 + frame_indention, y_frame, x2 - frame_indention, y_frame);
  ...
  public static void fill3DRectImproved(
    ...
    //     * @param lightDirDeg   Light direction in degrees (0° = right, 90° = down, 180° = left,
    // 270° = up)
  ...
  private final Color DECK_COLOR_GREEN =
      new Color(0, 100, 70); // MUST BE THE SAME AS IN DASBOARD_view_AWS_radar.java and
  // DASHBOARD_VIEW_APR_radar.java
  private final Color DECK_COLOR_LIGHT_GREEN =
      new Color(0, 204, 102); // MUST BE THE SAME AS IN DASBOARD_view_AWS_radar.java and
  // DASHBOARD_VIEW_APR_radar.java
  ...
  public static final Color CONTAINER_COLOR_VARIOUS =
      Color.BLACK; // stand-in for various random generated container colors!
  public static final Color CONTAINER_COLOR_GREEN =
      new Color(0, 128, 0); // Evergreen (https://www.freightcourse.com/shipping-container-colors/)
  ...
  public static final Color CONTAINER_COLOR_DARK_BLUE =
      new Color(0, 0, 204); // CMA CGM / Wan Hai Lines
  ...
  public static final Color CONTAINER_COLOR_MAROON =
      new Color(128, 0, 0); // HMM / Zim Integrated Shipping Services [Deep reddish brown]
  ...
  private final int BRIDGE_DRAW_LIMIT =
      1000; // if wind_rose_diameter > this value: draw bridge slightly different
}
